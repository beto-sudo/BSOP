...
export default function UsersPage() {
  ...
  const fetchUsers = () => {
    ...
    fetch(`/api/settings/users?companyId=${companyId}&query=${encodeURIComponent(q)}`)
      .then((r) => r.json())
      .then((res) => { setRows(res.rows || []); setCount(res.count || 0); })
      .finally(() => setLoading(false));
  };

  useEffect(() => { fetchUsers(); }, [companyId, q]);
  ...
  async function sendInvitation() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json || txt || "Error enviando invitación");

      const msgs: string[] = [];
      if (json?.invitationUrl) msgs.push(`Link: ${json.invitationUrl}`);
      if (json?.warning) msgs.push(json.warning);
      if (json?.memberWarning) msgs.push(json.memberWarning);
      if (json?.roleWarning) msgs.push(json.roleWarning);
      if (msgs.length === 0) msgs.push("Invitación enviada.");
      setInviteMsg(msgs.join(" • "));

      setInviteEmail("");
      fetchUsers(); // verás el pendiente aunque el member upsert haya fallado
    } catch (e: any) {
        ...
        <div className="border rounded-lg p-3 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              ...
              <input type="email" className="w-full border rounded px-2 py-1" placeholder="usuario@dominio.com"
            ...
            <div className="flex items-end">
              <button className="px-3 py-1.5 border rounded-lg text-sm hover:bg-gray-50" onClick={sendInvitation}
                disabled={inviting || !inviteEmail}>
                {inviting ? "Enviando..." : "Enviar invitación"}
              </button>
            </div>
          </div>
          {inviteMsg && <div className="text-sm text-gray-600 whitespace-pre-wrap">{inviteMsg}</div>}
        </div>
      )}

      <div className="flex gap-2">
        <input className="border rounded px-2 py-1 w-80" placeholder="Buscar por nombre o email"
