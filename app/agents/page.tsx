...
function StatSub({ loading, value }: { loading: boolean; value: string }) {
  if (loading) {
  ...
  const recentAgents = useMemo(
    () =>
      agents.recentAgents.filter((agent) => {
        ...
        const looksFinished = Boolean(
          agent.completionSummary || agent.completedAt || agent.resultSnippet
        );
          ...
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              <div className="rounded-3xl border border-white/8 bg-white/4 p-5">
                ...
                <div className="mt-2 text-2xl font-semibold text-white">{recentAgents.length}</div>
          ...
          <Surface className="p-6">
            ...
            <div className="grid gap-4">
              {recentAgents.map((agent) => {
                const modelHintSource =
                  `${agent.task ?? ''} ${agent.resultSnippet ?? ''}`.toLowerCase();
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="max-w-3xl">
                        ...
                        <p className="mt-2 text-sm leading-6 text-white/60">{agent.taskPreview}</p>
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                      {[
                        [
                          'Runtime',
                          agent.durationMinutes ? `${agent.durationMinutes.toFixed(1)}m` : '—',
                        ],
