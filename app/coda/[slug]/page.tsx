      ...
      <ContentShell>
        ...
        <div className="mb-8 flex flex-col gap-4 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <div className="mt-2 text-lg font-semibold text-[var(--text)]">
              {formatAuditTimestamp(document.lastAudit)}
            </div>
            <div className="mt-2 text-sm text-[var(--muted-foreground)]">
              Doc ID {document.docId}
            </div>
          </div>
          <Link
          ...
          {[
            {
              ...
              label: 'Tables',
              value: formatInt(document.stats.tables),
              sub: `${formatInt(document.stats.pages)} pages`,
            },
            {
              icon: Columns3,
              label: 'Columns',
              value: formatInt(document.stats.columns),
              sub: 'Total schema width',
            },
            {
              icon: Network,
              label: 'Relationships',
              value: formatInt(document.stats.relationships),
              sub: 'Cross-table connections',
            },
            {
              icon: ShieldAlert,
              label: 'Avg health score',
              value: document.health.avgScore.toFixed(2),
              sub: `Max score ${document.health.maxScore}`,
              color: healthColor,
            },
            {
              icon: Blocks,
              label: 'God tables',
              value: formatInt(document.health.godTables),
              sub: `${formatInt(document.health.highRiskCount)} high-risk tables`,
            },
            {
              icon: Sparkles,
              label: 'KPI suggestions',
              value: formatInt(document.health.kpiSuggestions),
              sub: `${formatInt(document.health.duplicateGroups)} duplicate groups`,
            },
          ].map((item) => (
            <Surface key={item.label} className="p-5">
              ...
              <div
                ...
                style={item.color ? { color: item.color } : { color: 'var(--text)' }}
              >
                {item.value}
              </div>
        ...
        <section className="mt-10 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--muted-foreground)]">
                  Highest-scoring health risks in the latest audit snapshot.
                </p>
              </div>
            </div>
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {document.topRiskTables.map((table) => (
                    <tr
                      key={table.name}
                      className="border-b border-[var(--border)] align-top text-[var(--text)]/78 last:border-0"
                    >
                      <td className="py-4 pr-4 font-medium text-[var(--text)]">{table.name}</td>
                      <td className="py-4 pr-4 text-[var(--text)]/65">
                        {formatInt(table.columnCount)}
                      </td>
                      <td
                        className="py-4 pr-4 font-semibold"
                        style={{ color: getHealthColor(table.healthScore) }}
                      >
                        {table.healthScore}
                      </td>
                      <td className="py-4">
                        <div className="flex flex-wrap gap-2">
                          {table.findings.length ? (
                            table.findings.map((finding) => (
                              <span
                                key={finding}
                                className="rounded-full border border-[var(--border)] bg-[var(--panel)] px-3 py-1 text-xs text-[var(--text)]/72"
                              >
                                {finding}
                              </span>
                            ))
          ...
          <Surface className="p-6">
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              Top 10 oversized tables by column count.
            </p>
            <div className="mt-5 space-y-3">
              {document.godTablesList.map((table, index) => (
                ...
                >
                  <div>
                    <div className="text-xs uppercase tracking-[0.18em] text-[var(--text)]/35">
                      #{index + 1}
                    </div>
        ...
        <section className="mt-10">
          <div className="mb-5">
            ...
            <p className="mt-2 text-sm text-[var(--muted-foreground)]">
              Functional clusters inferred from the audit’s structure analysis.
            </p>
          </div>
