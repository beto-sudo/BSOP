    ...
    <RequireAccess empresa="coda">
      <ContentShell>
          ...
          {[
            {
              ...
              value: formatInt(totalDocuments),
              sub: 'Five operating documents tracked',
            },
            {
              icon: LayoutDashboard,
              label: 'Tables',
              value: formatInt(codaData.totals.totalTables),
              sub: 'Across all audited documents',
            },
            {
              icon: Grid2x2,
              label: 'Columns',
              value: formatInt(codaData.totals.totalColumns),
              sub: 'Schema breadth in production',
            },
            {
              icon: Network,
              label: 'Relationships',
              value: formatInt(codaData.totals.totalRelationships),
              sub: 'Detected cross-table links',
            },
            {
              icon: ShieldAlert,
              label: 'God tables',
              value: formatInt(codaData.totals.totalGodTables),
              sub: 'Oversized risk concentration',
            },
            {
              icon: Sparkles,
              label: 'KPI suggestions',
              value: formatInt(codaData.totals.totalKpiSuggestions),
              sub: 'Metrics opportunities identified',
            },
          ].map((item) => (
            <Surface key={item.label} className="p-5">
              <item.icon className="h-5 w-5 text-amber-300" />
              <div className="mt-4 text-xs uppercase tracking-[0.24em] text-[var(--text)]/40">
                {item.label}
              </div>
              <div className="mt-2 text-3xl font-semibold text-[var(--text)]">{item.value}</div>
        ...
        <section className="mt-10">
          <div className="mb-5">
            <h2 className="text-2xl font-semibold text-[var(--text)]">How Coda Architect Works</h2>
            <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--muted-foreground)]">
              The audit pipeline starts with raw schema extraction, then moves through structural
              analysis, health scoring, risk detection, and reporting.
            </p>
          </div>
              ...
              return (
                <Surface key={step.step} className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-[var(--border)] bg-[var(--panel)] text-amber-300">
                  ...
                  <h3 className="mt-4 text-lg font-semibold text-[var(--text)]">{step.name}</h3>
        ...
        <section className="mt-10">
          <div className="mb-5">
            <h2 className="text-2xl font-semibold text-[var(--text)]">Audit types</h2>
            <p className="mt-2 text-sm leading-7 text-[var(--muted-foreground)]">
              Three levels of coverage depending on whether you need a quick pulse, a full
              architecture review, or change tracking.
            </p>
          </div>
        ...
        <section className="mt-10">
          <div className="mb-5">
            <h2 className="text-2xl font-semibold text-[var(--text)]">Documents</h2>
            <p className="mt-2 text-sm leading-7 text-[var(--muted-foreground)]">
              Each card links to a document-specific dashboard with top risk tables, god tables, and
              module distribution.
            </p>
          </div>
              ...
              return (
                <Surface key={doc.slug} className="p-6">
                  <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <div className="text-xs uppercase tracking-[0.24em] text-[var(--text)]/40">
                        {doc.slug}
                      </div>
                      <h3 className="mt-2 text-2xl font-semibold text-[var(--text)]">{doc.name}</h3>
                      <p className="mt-3 text-sm leading-7 text-[var(--muted-foreground)]">
                        {doc.description}
                      </p>
                    </div>
                    <div className="min-w-36 rounded-2xl border border-[var(--border)] bg-[var(--panel)] px-4 py-3 text-right">
                      <div className="text-xs uppercase tracking-[0.22em] text-[var(--text)]/45">
                        Avg health
                      </div>
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {[
                      [
                        'Tables / Columns',
                        `${formatInt(doc.stats.tables)} / ${formatInt(doc.stats.columns)}`,
                      ],
                      ...
                      >
                        ...
                        <div className="mt-2 text-sm font-medium text-[var(--text)]/80">
                          {value}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="mt-6 flex items-center justify-between gap-4 border-t border-[var(--border)] pt-5">
                    <div className="text-sm text-[var(--muted-foreground)]">
                      Top risk table: {doc.topRiskTables[0]?.name ?? 'None'}
                    </div>
                    <Link
                      href={`/coda/${doc.slug}`}
                      className="inline-flex items-center gap-2 text-sm font-medium text-[var(--accent-soft)] transition hover:text-[var(--text)]"
                    >
                      View Dashboard
