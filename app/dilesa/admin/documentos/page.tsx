...
function DocumentosInner() {
  ...
  const fetchDocumentos = useCallback(async () => {
    const { data, error: err } = await supabase.schema('erp' as any).from('documentos').select('*').eq('empresa_id', EMPRESA_ID).is('deleted_at', null).order('created_at', { ascending: false });
    if (err) { setError(err.message); return; }
    setDocumentos(data ?? []);
  }, [supabase]);

  const fetchNotarias = useCallback(async () => {
    const { data: provData } = await supabase.schema('erp' as any).from('proveedores').select('id, persona_id, empresa_id').eq('empresa_id', EMPRESA_ID).eq('categoria', 'notaria').eq('activo', true).is('deleted_at', null);
  ...
  const handleCreateNotaria = async () => {
    ...
    try {
      const { data: persona, error: pe } = await supabase.schema('erp' as any).from('personas').insert({ empresa_id: EMPRESA_ID, nombre: newNotariaNombre.trim(), tipo: 'proveedor' }).select('id').single();
      if (pe) throw pe;
      const { data: prov, error: pre } = await supabase.schema('erp' as any).from('proveedores').insert({ empresa_id: EMPRESA_ID, persona_id: persona.id, categoria: 'notaria', activo: true }).select('id, empresa_id').single();
      if (pre) throw pre;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--text)]">Documentos — DILESA</h1>
