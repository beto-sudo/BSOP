...
type Empleado = {
  id: string; empresa_id: string; numero_empleado: string | null; fecha_ingreso: string | null;
  fecha_baja: string | null; activo: boolean;
  persona: { nombre: string; apellido_paterno: string | null; apellido_materno: string | null; email: string | null } | null;
  departamento: { nombre: string } | null; puesto: { nombre: string } | null;
};
...
function fullName(emp: Empleado) {
  ...
  return [emp.persona.nombre, emp.persona.apellido_paterno, emp.persona.apellido_materno].filter(Boolean).join(' ');
}

function formatDate(d: string | null) {
  if (!d) return '—';
  return new Date(d.includes('T') ? d : `${d}T00:00:00`).toLocaleDateString('es-MX', { day: '2-digit', month: 'short', year: 'numeric' });
}
...
function EmpleadosInner() {
  ...
  const [createForm, setCreateForm] = useState({ persona_id: '', departamento_id: '', puesto_id: '', numero_empleado: '', fecha_ingreso: '' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--text)]">Empleados — DILESA</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={() => setShowCreate(true)} className="rounded-xl bg-[var(--accent)] text-white hover:bg-[var(--accent)]/90 gap-1.5"><Plus className="h-4 w-4" />Nuevo empleado</Button>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex rounded-xl border border-[var(--border)] bg-[var(--panel)] p-1 gap-1">
            {(['activos', 'inactivos'] as const).map((t) => (
              <button key={t} type="button" onClick={() => setTab(t)} className={['rounded-lg px-4 py-1.5 text-xs font-semibold capitalize transition', tab === t ? 'bg-[var(--accent)] text-white shadow-sm' : 'text-[var(--text)]/60 hover:text-[var(--text)]'].join(' ')}>{t === 'activos' ? 'Activos' : 'Ex-empleados'}</button>
            ))}
          </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]">
        {error ? (
          ...
          <div className="divide-y divide-[var(--border)]">{Array.from({ length: 5 }).map((_, i) => (<div key={i} className="flex items-center gap-4 p-4"><Skeleton className="h-9 w-9 rounded-full" /><Skeleton className="h-4 w-48" /><Skeleton className="h-4 w-28 ml-auto" /><Skeleton className="h-4 w-24" /></div>))}</div>
        ) : visible.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-16 text-center"><Users className="mb-3 h-10 w-10 text-[var(--text)]/20" /><p className="text-sm text-[var(--text)]/55">{empleados.length === 0 ? 'No hay empleados registrados' : 'Sin resultados para los filtros actuales'}</p></div>
        ) : (
          <Table>
            ...
            <TableBody>
              {sortData(visible.map((emp) => ({ ...emp, nombre: fullName(emp) || null, departamento_nombre: emp.departamento?.nombre ?? null, puesto_nombre: emp.puesto?.nombre ?? null }))).map((emp) => (
                <TableRow key={emp.id} className="cursor-pointer border-[var(--border)] hover:bg-[var(--panel)] transition-colors" onClick={() => router.push(`/dilesa/rh/empleados/${emp.id}`)}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--accent)]/15 text-xs font-semibold text-[var(--accent)]">{(emp.persona?.nombre?.[0] ?? '?').toUpperCase()}</div>
        ...
        <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto rounded-3xl border-[var(--border)] bg-[var(--card)] text-[var(--text)]">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><FieldLabel>Departamento</FieldLabel><Select value={createForm.departamento_id} onValueChange={(v) => setCreateForm((f) => ({ ...f, departamento_id: v ?? '' }))}><SelectTrigger className="rounded-xl border-[var(--border)] bg-[var(--panel)] text-[var(--text)]"><SelectValue placeholder="Sin departamento" /></SelectTrigger><SelectContent>{departamentos.map((d) => (<SelectItem key={d.id} value={d.id}>{d.nombre}</SelectItem>))}</SelectContent></Select></div>
              <div><FieldLabel>Puesto</FieldLabel><Select value={createForm.puesto_id} onValueChange={(v) => setCreateForm((f) => ({ ...f, puesto_id: v ?? '' }))}><SelectTrigger className="rounded-xl border-[var(--border)] bg-[var(--panel)] text-[var(--text)]"><SelectValue placeholder="Sin puesto" /></SelectTrigger><SelectContent>{puestos.map((p) => (<SelectItem key={p.id} value={p.id}>{p.nombre}</SelectItem>))}</SelectContent></Select></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><FieldLabel>No. Empleado</FieldLabel><Input placeholder="Ej: EMP-001" value={createForm.numero_empleado} onChange={(e) => setCreateForm((f) => ({ ...f, numero_empleado: e.target.value }))} className="rounded-xl border-[var(--border)] bg-[var(--panel)] text-[var(--text)]" /></div>
