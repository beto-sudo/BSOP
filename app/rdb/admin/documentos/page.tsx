 ...
 * Cleanup PR (#30): pre-existing debt. `any` is used for dynamic document
 * metadata (Record<string, any>) crossing Supabase JSON boundaries; proper
 * typing requires a schema-wide refactor. set-state-in-effect data-sync
 * patterns are behavioral — rewriting changes render semantics, out of scope.
 */
...
function ActaConstitutivaFields({
  meta,
  onChange,
}: {
  meta: Record<string, any>;
    ...
    <div className="space-y-3 rounded-xl border border-[var(--border)] bg-[var(--panel)]/50 p-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Select
            value={meta.tipo_poder || undefined}
            onValueChange={(v) => onChange({ ...meta, tipo_poder: v })}
          >
  ...
  const handleCreateNotaria = async () => {
    ...
    try {
      const { data: persona, error: pe } = await supabase
        .schema('erp')
        .from('personas')
        .insert({ empresa_id: EMPRESA_ID, nombre: newNotariaNombre.trim(), tipo: 'proveedor' })
        .select('id')
        .single();
      if (pe) throw pe;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--text)]">
            Documentos — Rincón del Bosque
          </h1>
          <p className="mt-1 text-sm text-[var(--text)]/55">
            Escrituras, contratos, seguros y documentos legales
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
