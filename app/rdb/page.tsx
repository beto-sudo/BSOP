...
const MONTH_FMT = new Intl.DateTimeFormat('es-MX', {
  timeZone: TZ,
  month: 'long',
  year: 'numeric',
});
...
function buildMonthComparisonChart(
  currentRows: PedidoLite[],
  previousRows: PedidoLite[],
  lastYearRows: PedidoLite[],
  referenceDate: Date
): ChartPoint[] {
  ...
  const lastYear = aggregateMonthByDay(
    lastYearRows,
    new Date(referenceDate.getFullYear() - 1, referenceDate.getMonth(), 1)
  );
...
function SalesChart({ data, showComparisons }: { data: ChartPoint[]; showComparisons: boolean }) {
  ...
  const values = data.flatMap((point) => [
    point.current ?? 0,
    showComparisons ? (point.previous ?? 0) : 0,
    showComparisons ? (point.lastYear ?? 0) : 0,
  ]);
  const maxValue = Math.max(...values, 1);
  const currentPath = buildSeriesPath(
    data.map((point) => point.current),
    width,
    height,
    maxValue
  );
  const previousPath = buildSeriesPath(
    data.map((point) => point.previous),
    width,
    height,
    maxValue
  );
  const lastYearPath = buildSeriesPath(
    data.map((point) => point.lastYear),
    width,
    height,
    maxValue
  );
  ...
  return (
    <div className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-4 sm:p-5">
      <div className="mb-4 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2 text-sm text-[var(--text)]/65">
      ...
      <div className="overflow-x-auto">
        <svg viewBox={`0 0 ${width} ${height + 28}`} className="min-w-[720px]">
          ...
          />
          {data
            .filter((point) => point.day === 1 || point.day % 5 === 0 || point.day === data.length)
            .map((point) => {
              const x =
                data.length === 1 ? width / 2 : ((point.day - 1) / (data.length - 1)) * width;
              return (
                <text
                  key={point.day}
                  x={x}
...
export default function RdbHomePage() {
  ...
  const fetchHome = useCallback(async () => {
    ...
    try {
      ...
      const cortesQuery = supabase
        .schema('rdb')
        .from('v_cortes_lista')
        .select(
          'id,corte_nombre,caja_nombre,fecha_operativa,hora_inicio,hora_fin,estado,efectivo_contado,efectivo_esperado,total_ingresos,pedidos_count'
        )
  ...
  const cleanRows = useMemo(
    () => currentRows.filter((row) => !isCancelled(row.status)),
    [currentRows]
  );
  const sales = useMemo(() => sumSales(cleanRows), [cleanRows]);
  const orders = cleanRows.length;
  const avgTicket = orders ? sales / orders : 0;
  const openCorte = useMemo(
    () => cortes.find((corte) => normalizeStatus(corte.estado) === 'abierto'),
    [cortes]
  );
  ...
  const insights = useMemo(
    () => buildInsights(cleanRows, chartData, meta.rangeLabel),
    [cleanRows, chartData, meta.rangeLabel]
  );
  ...
  const attention = useMemo<AttentionAlert[]>(() => {
    ...
    if (desfasados) {
      const diff = Math.abs(
        (desfasados.efectivo_contado ?? 0) - (desfasados.efectivo_esperado ?? 0)
      );
      alerts.push({
    ...
    const sinPedidos = cortes.find(
      (corte) =>
        normalizeStatus(corte.estado) === 'cerrado' &&
        (corte.total_ingresos ?? 0) > 0 &&
        (corte.pedidos_count ?? 0) === 0
    );
    if (sinPedidos) {
      alerts.push({
  ...
  const previousTotal = useMemo(
    () => sumSales(previousRows.filter((row) => !isCancelled(row.status))),
    [previousRows]
  );
  const lastYearTotal = useMemo(
    () => sumSales(lastYearRows.filter((row) => !isCancelled(row.status))),
    [lastYearRows]
  );
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-[var(--text)]/60">
                Lectura rápida del negocio, enfocada en ventas del periodo y estado de cortes, sin
                duplicar accesos del menú.
              </p>
            </div>
        ...
        {loading ? (
          <div className="flex min-h-[240px] items-center justify-center rounded-3xl border border-[var(--border)] bg-[var(--card)]">
            <div className="flex items-center gap-3 text-[var(--text)]/60">
              <Loader2 className="h-4 w-4 animate-spin" /> Cargando home de RDB…
            </div>
          </div>
        ) : error ? (
          ...
          <>
            <section className="space-y-3">
              ...
              {attention.length ? (
                <div className="grid gap-3 xl:grid-cols-3">
                  {attention.map((alert) => (
                    <AttentionCard key={`${alert.title}-${alert.detail}`} alert={alert} />
                  ))}
                </div>
              ) : (
                <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-300">
                  Sin alertas confiables por ahora. Lo operativo visible se ve estable.
                </div>
              )}
            </section>
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <KpiCard
                ...
                hint={meta.rangeLabel}
                icon={<TrendingUp className="h-4 w-4" />}
              />
              <KpiCard
                label="Pedidos del periodo"
                value={String(orders)}
                hint={`${safeCount(cleanRows)} registrados`}
                icon={<ShoppingBag className="h-4 w-4" />}
              />
              <KpiCard
                label="Ticket promedio"
                value={formatMoney(avgTicket)}
                hint={orders ? `${orders} pedidos considerados` : 'Sin pedidos en el rango'}
                icon={<Receipt className="h-4 w-4" />}
              />
              <KpiCard
                label="Estado de cortes"
                value={
                  openCorte ? 'Corte abierto' : latestCorte ? 'Sin corte abierto' : 'Sin datos'
                }
                hint={
                  openCorte
                    ? `${openCorte.corte_nombre ?? 'Corte actual'}${openCorte.caja_nombre ? ` · ${openCorte.caja_nombre}` : ''}`
                    : latestCorte
                      ? `Último: ${latestCorte.corte_nombre ?? 'corte reciente'}`
                      : undefined
                }
            ...
            <section className="space-y-4">
              ...
              {range === 'month' ? (
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
                    <div className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text)]/45">
                      Vs mes anterior
                    </div>
                  ...
                  <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4 md:col-span-2">
                    ...
                    <div className="mt-2 text-sm text-[var(--text)]/65">
                      Comparativo visual diario contra el mes anterior y el mismo mes del año
                      pasado.
                    </div>
                  </div>
                </div>
