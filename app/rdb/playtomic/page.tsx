...
const PENDING_DATE_FMT = new Intl.DateTimeFormat('en-GB', {
  timeZone: TZ,
  day: '2-digit',
  month: '2-digit',
  year: 'numeric',
});
const PENDING_TIME_FMT = new Intl.DateTimeFormat('en-GB', {
  timeZone: TZ,
  hour: '2-digit',
  minute: '2-digit',
  hour12: false,
...
export default function PlaytomicPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const revenueTotal = data.bookings
      .filter(
        (booking) =>
          !booking.is_canceled && !(booking.status ?? '').toLowerCase().includes('cancel')
      )
  ...
  const cancellationAnalysis = useMemo(() => {
    ...
    return {
      ...
      cancellationRate: data.bookings.length
        ? (canceledBookings.length / data.bookings.length) * 100
        : 0,
      avgCanceledDuration: canceledDurationCount
        ? canceledDurationTotal / canceledDurationCount
        : 0,
      sports,
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-[var(--text)]/60">
                Reservas, ingresos, ocupación, jugadores y salud de sincronización en una sola
                vista.
              </p>
            </div>
          ...
          <>
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              <KpiCard
                ...
                value={formatMoney(reconciliation.summary.pendingRevenue)}
              ...
              <div className="space-y-4 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-4 sm:p-5">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    ...
                    <Select
                      value={playerSort}
                      onValueChange={(value) => setPlayerSort(value as PlayerSortKey)}
                    >
                    ...
                    <TableBody>
                      {topPlayers.length === 0 ? (
                        <TableRow>
                          <TableCell
                            colSpan={4}
                            className="py-10 text-center text-[var(--text)]/50"
                          >
                        ...
                        topPlayers.slice(0, 10).map((player) => (
                          <TableRow
                            key={`${player.email ?? 'sin-correo'}-${player.name ?? 'sin-nombre'}`}
                          >
                            <TableCell>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-2xl border border-rose-500/20 bg-[var(--card)] p-4">
                  ...
                  <div className="mt-2 text-3xl font-semibold text-[var(--text)]">
                    {cancellationAnalysis.canceledCount}
                  </div>
                ...
                <div className="rounded-2xl border border-rose-500/20 bg-[var(--card)] p-4">
                  ...
                  <div className="mt-2 text-3xl font-semibold text-[var(--text)]">
                    {cancellationAnalysis.cancellationRate.toFixed(1)}%
                  </div>
                  <div className="mt-1 text-sm text-[var(--text)]/55">
                    Sobre {data.bookings.length} reservas del periodo.
                  </div>
                </div>
                <div className="rounded-2xl border border-rose-500/20 bg-[var(--card)] p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-rose-500">
                  ...
                  <div className="mt-2 space-y-1 text-sm text-[var(--text)]">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-medium">
                        {cancellationAnalysis.sports.PADEL.canceled} (
                        {cancellationAnalysis.sports.PADEL.total
                          ? (
                              (cancellationAnalysis.sports.PADEL.canceled /
                                cancellationAnalysis.sports.PADEL.total) *
                              100
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <span>Tennis</span>
                      <span className="font-medium">
                        {cancellationAnalysis.sports.TENNIS.canceled} (
                        {cancellationAnalysis.sports.TENNIS.total
                          ? (
                              (cancellationAnalysis.sports.TENNIS.canceled /
                                cancellationAnalysis.sports.TENNIS.total) *
                              100
                ...
                <div className="rounded-2xl border border-rose-500/20 bg-[var(--card)] p-4">
                  ...
                  <div className="mt-2 text-3xl font-semibold text-[var(--text)]">
                    {cancellationAnalysis.avgCanceledDuration.toFixed(0)} min
                  </div>
                  <div className="mt-1 text-sm text-[var(--text)]/55">
            ...
            <section className="space-y-4 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-4 sm:p-5">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  <p className="text-sm text-[var(--text)]/55">
                    Desglose diario de reservas por estado de pago y origen para cuadrar contra
                    depósitos.
                  </p>
                </div>
                <Button
              ...
              {reconciliation.truncated ? (
                <p className="text-sm text-[var(--text)]/55">
                  Mostrando 60 de {reconciliation.totalDays} días. Los totales reflejan el periodo
                  completo.
                </p>
              ) : null}
                            ...
                            <>
                              {pendingPayments.playerSummary.slice(0, 20).map((player) => (
                                <TableRow key={`${player.jugador}-${player.email}`}>
                                  ...
                                  <TableCell className="text-[var(--text)]/60">
                                    {player.email}
                                  </TableCell>
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {pendingPayments.detailRows.length === 0 ? (
                                ...
                                pendingSortData(pendingPayments.detailRows).map(
                                  (booking, index) => (
                                    <TableRow
                    ...
                    <div className="flex flex-col items-end gap-1 text-sm">
                      {lastSuccess ? (
                        <span className="text-green-600 dark:text-green-400">
                          ✓ Último éxito:{' '}
                          {formatDateTime(lastSuccess.finished_at ?? lastSuccess.started_at)}
                      ...
                      {lastError ? (
                        <span className="text-red-500 dark:text-red-400 max-w-md truncate">
                          ✗ Último error:{' '}
                          {formatDateTime(lastError.finished_at ?? lastError.started_at)}
