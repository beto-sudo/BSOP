      ...
      <ContentShell>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {(
                [
                  ...
                  [
                    t('rnd.stat.coverage'),
                    t('rnd.stat.focus_areas', { count: councilData.config.scope.length }),
                  ],
                  [
                    t('rnd.stat.council_size'),
                    t('rnd.stat.models', { count: councilMembers.length }),
                  ],
                ] as [string, string][]
              ).map(([label, value]) => (
                <Surface key={label} className="border-amber-300/15 bg-[var(--bg)]/20 p-5">
                  <div className="text-xs uppercase tracking-[0.24em] text-[var(--text)]/35">
                    {label}
                  </div>
        ...
        <section className="mt-10">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            {councilMembers.map((member) => (
              <Surface
                key={member.id}
                className={`p-5 ${memberTone[member.id] ?? 'border-[var(--border)] bg-[var(--card)]'}`}
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="text-3xl">{member.emoji}</div>
                  <div className="rounded-full border border-[var(--border)] bg-[var(--card)] px-3 py-1 text-xs font-medium text-[var(--text)]/75">
                    {member.model}
                  </div>
                </div>
        ...
        <section className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {(
            [
              [
                t('rnd.stats.total_sessions'),
                String(stats.totalSessions),
                t('rnd.stats.total_sessions_sub'),
              ],
              [t('rnd.stats.ideas'), String(stats.totalIdeas), t('rnd.stats.ideas_sub')],
              [
                t('rnd.stats.implemented'),
                String(stats.implementedIdeas),
                t('rnd.stats.implemented_sub'),
              ],
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg)]/10 px-4 py-3">
                        ...
                        <div className="mt-2 text-sm font-medium text-[var(--text)]/80">
                          {t('rnd.archive.proposals', { count: memo.ideas?.length ?? 0 })}
                        </div>
                      </div>
                      <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg)]/10 px-4 py-3">
                        <div className="text-xs uppercase tracking-[0.2em] text-[var(--text)]/35">
                          {t('rnd.archive.recommendations')}
                        </div>
                        <div className="mt-2 text-sm font-medium text-[var(--text)]/80">
                          {t('rnd.archive.ranked', { count: memo.recommendations?.length ?? 0 })}
                        </div>
                      </div>
                    </div>
                  </Surface>
          ...
          <div>
            <div className="mb-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <Link
                  href={`/rnd/${selectedMemo.id}`}
                  className="shrink-0 text-sm font-medium text-amber-300 transition hover:text-[var(--text)]"
                >
                  {t('rnd.latest.open')}
                </Link>
              </div>
              <div className="mt-3 flex items-center gap-3">
                <label
                  htmlFor="memo-selector"
                  className="text-xs uppercase tracking-[0.2em] text-[var(--text)]/35"
                >
            ...
            <Surface className="overflow-hidden p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span
                  className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-medium ${badgeTone[selectedMemo.status] ?? badgeTone.running}`}
                >
                  <span className="h-2 w-2 animate-pulse rounded-full bg-amber-300" />
                  {selectedMemo.status}
                </span>
              ...
              <section className="mt-8">
                <h4 className="text-sm font-semibold uppercase tracking-[0.22em] text-[var(--text)]/45">
                  {t('rnd.memo.ideas_section')}
                </h4>
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <h5 className="mt-2 text-lg font-semibold text-[var(--text)]">
                              {idea.title}
                            </h5>
                          </div>
                          <div className="flex gap-2 text-xs text-[var(--text)]/70">
                            <span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1">
                              {t('rnd.memo.impact', { score: idea.impact })}
                            </span>
                            <span className="rounded-full border border-sky-400/20 bg-sky-400/10 px-3 py-1">
                              {t('rnd.memo.effort', { score: idea.effort })}
                            </span>
                          </div>
                        </div>
                ...
                <div className="mt-4 space-y-4">
                  {selectedDebate.map((round) => {
                    ...
                    return (
                      <div
                        key={round.round}
                        className="rounded-3xl border border-[var(--border)] bg-[var(--bg)]/10 p-5"
                      >
                        <div className="text-xs uppercase tracking-[0.2em] text-[var(--text)]/35">
                          {t('rnd.memo.round', { n: round.round, topic: roundTopic })}
                        </div>
                          ...
                          {(round.highlights ?? []).map((entry, index) => (
                            ...
                            >
                              <div className="text-sm font-medium text-[var(--text)]">
                                {councilById[entry.speaker]?.emoji}{' '}
                                {councilById[entry.speaker]?.name}
                              </div>
                      ...
                      >
                        ...
                        <ul className="mt-4 space-y-2 text-sm text-[var(--muted)]">
                          {recommendationActions.map((item) => (
                            <li
                              key={item}
                              className="flex gap-3 rounded-2xl border border-[var(--border)] bg-[var(--bg)]/10 px-4 py-3"
                            >
