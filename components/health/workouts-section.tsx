    ...
    <section className="mt-10 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
      <Surface className="p-6 shadow-sm dark:shadow-none">
        <div className="mb-4 flex items-center gap-3 text-[var(--text)] dark:text-white">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted-foreground)] dark:text-white/45">
              Filtered by the active date range.
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatPill
            label="Distance"
            value={`${formatMetricValue(workoutSummary.distance, 1)} km`}
          />
        ...
        {workouts.length ? (
          <div className="space-y-4">
            {workouts.slice(0, 5).map((workout) => (
              <div
                key={`${workout.name}-${workout.start_time}`}
                className="rounded-3xl border border-[var(--border)] bg-[var(--panel)] p-5 dark:border-white/8 dark:bg-white/4"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-2 text-sm text-[var(--muted-foreground)] dark:text-white/45">
                  ...
                  <div className="rounded-full border border-[var(--border)] bg-[var(--card)] px-3 py-1 text-xs text-[var(--muted-foreground)] dark:border-white/10 dark:bg-black/20 dark:text-white/60">
                    {workout.source ?? 'Unknown source'}
                  </div>
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-4">
                  {[
                    ...
                    [
                      'Energy',
                      workout.energy_kcal == null ? '—' : `${Math.round(workout.energy_kcal)} kcal`,
                    ],
                    [
                      'Distance',
                      workout.distance_km == null ? '—' : `${workout.distance_km.toFixed(1)} km`,
                    ],
                    [
                      'Avg HR',
                    ...
                    <div
                      key={label}
                      className="rounded-2xl border border-[var(--border)] bg-[var(--card)] px-4 py-3 dark:border-white/6 dark:bg-black/10"
                    >
                      <div className="text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)] dark:text-white/35">
                        {label}
                      </div>
                      <div className="mt-2 text-sm font-medium text-[var(--text)] dark:text-white/85">
                        {value}
                      </div>
                    </div>
                  ))}
          ...
          <EmptyState
            ...
            copy="Once workout exports arrive inside the selected window, sessions will show here with duration, energy, distance, and heart rate."
          />
        )}
      </Surface>
        ...
        {workoutSummary.mix.length ? (
          <div className="space-y-4">
            <div className="rounded-3xl border border-[var(--border)] bg-[var(--panel)] p-5 dark:border-white/8 dark:bg-white/4">
              ...
              <div className="mt-2 text-2xl font-semibold text-[var(--text)] dark:text-white">
                {workoutSummary.mix[0]?.[0]}
              </div>
              <div className="mt-1 text-sm text-[var(--muted-foreground)] dark:text-white/50">
                {workoutSummary.mix[0]?.[1]} sessions in {rangeLabel.toLowerCase()}
              </div>
            </div>
          ...
          <EmptyState
            title="No workout mix yet"
            copy="Workout distribution will appear here once the selected range includes sessions."
          />
        )}
      </Surface>
    </section>
