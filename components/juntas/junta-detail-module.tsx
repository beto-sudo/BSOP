...
/**
 * JuntaDetailModule — módulo cross-empresa de detalle de junta (admin).
 *
 * Convención SM1-SM5 (ADR-011): el page de cada empresa es un wrapper
 * de ~30 líneas que delega aquí. La diferencia entre empresas es solo el
 * `empresaSlug` para construir la URL de retorno del breadcrumb / delete.
 *
 * Usado por:
...
export type JuntaDetailModuleProps = {
  /**
   * Slug en URL (`'rdb' | 'dilesa'`). Se usa para construir la URL de retorno
   * del botón "Volver" y del redirect post-eliminar.
   */
  empresaSlug: 'rdb' | 'dilesa';
};

export function JuntaDetailModule({ empresaSlug }: JuntaDetailModuleProps) {
  ...
  const juntasListUrl = `/${empresaSlug}/admin/juntas`;
  ...
  }, [junta?.id, estado, editor, supabase, junta]);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(juntasListUrl)}
          ...
          <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-3">
              <label
                ...
                title="Al terminar la junta, enviar también la minuta al correo del consejo configurado para esta empresa."
              >
                <input
                  type="checkbox"
