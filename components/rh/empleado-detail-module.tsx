...
/**
 * EmpleadoDetailModule — detalle de empleado parametrizado por empresa.
 *
 * Convención SM2 (ADR-011): page por empresa = wrapper delgado que
 * delega a este componente. Sub-PR 5 de `shared-modules-refactor`
 * (2026-04-27) consolida la versión completa de DILESA como base
 * canónica — RDB heredera todas las features (beneficiarios,
 * documentos, notas, contrato/finiquito, baja con generación
 * de finiquito).
 *
 * Política (Beto, 2026-04-27): no hay generación de contrato/finiquito
 * sin que la empresa tenga datos fiscales completos en `core.empresas`.
 * Los botones quedan deshabilitados con tooltip explicativo si los
 * datos están incompletos. Ver `lib/rh/datos-fiscales-empresa.ts`.
 */
...
export type EmpleadoDetailModuleProps = {
  /** Slug de la empresa para construir links de regreso. */
  empresaSlug: 'rdb' | 'dilesa';
};

function EmpleadoDetailInner({ empresaSlug }: EmpleadoDetailModuleProps) {
  ...
  const datosOk = datosFiscales.completo;
  const tooltipDatosFaltantes =
    datosFiscales.faltantes.length > 0
      ? `Faltan datos fiscales de la empresa: ${datosFiscales.faltantes.slice(0, 3).join(', ')}${
          datosFiscales.faltantes.length > 3 ? '…' : ''
        }. Captúralos en Configuración → Empresas.`
      : '';
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(`/${empresaSlug}/rh/personal`)}
        ...
        {isAdmin && (
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              disabled={!datosOk}
              title={datosOk ? 'Generar contrato individual de trabajo' : tooltipDatosFaltantes}
            ...
            {isBaja && (
              <Button
                variant="outline"
                onClick={() => router.push(`/${empresaSlug}/rh/personal/${empleado.id}/finiquito`)}
                disabled={!datosOk}
                title={
                  datosOk ? 'Generar convenio de terminación y finiquito' : tooltipDatosFaltantes
                }
                className="gap-1.5 rounded-xl border-[var(--border)] text-[var(--text)] disabled:opacity-50"
              >
      ...
      {/* Aviso de datos fiscales incompletos (admin only) */}
      {isAdmin && !datosFiscales.loading && !datosOk && (
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 text-xs text-amber-400">
          ⚠️ La empresa tiene datos fiscales incompletos — no se pueden generar contratos ni
          finiquitos. Faltan: <strong>{datosFiscales.faltantes.join(', ')}</strong>. Captúralos en{' '}
          <a
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              ...
              <Input
                ...
                placeholder={`nombre@${empresaSlug}.com`}
                className="rounded-xl border-[var(--border)] bg-[var(--panel)] text-[var(--text)]"
              />
            </div>
            <div>
            ...
            <div className="sm:col-span-2 lg:col-span-3">
              ...
              <Input
                ...
                placeholder="Oficinas Piedras Negras / obra en turno"
                className="rounded-xl border-[var(--border)] bg-[var(--panel)] text-[var(--text)]"
              />
            </div>
...
export function EmpleadoDetailModule({ empresaSlug }: EmpleadoDetailModuleProps) {
  return (
    <RequireAccess empresa={empresaSlug}>
      <EmpleadoDetailInner empresaSlug={empresaSlug} />
    </RequireAccess>
  );
}
