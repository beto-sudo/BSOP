    ...
    <Surface className="p-6">
      ...
      <form onSubmit={onAddExpense} className="mt-5 grid gap-3">
        <input
          ...
          onChange={(e) =>
            onExpenseDraftChange((current) => ({ ...current, concept: e.target.value }))
          }
          placeholder="Concepto"
          className="rounded-2xl border border-white/10 bg-black/10 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30"
        />
        <select
          value={expenseDraft.category}
          onChange={(e) =>
            onExpenseDraftChange((current) => ({ ...current, category: e.target.value }))
          }
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <input
            ...
            onChange={(e) =>
              onExpenseDraftChange((current) => ({ ...current, amount: e.target.value }))
            }
            placeholder="Monto"
            className="rounded-2xl border border-white/10 bg-black/10 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30"
          />
          <select
            ...
            onChange={(e) =>
              onExpenseDraftChange((current) => ({
                ...
                exchangeRate: e.target.value === baseCurrency ? '1' : String(defaultExchangeRate),
              }))
            }
            className="rounded-2xl border border-white/10 bg-black/10 px-4 py-3 text-sm text-white outline-none"
          >
            <option value="MXN" className="bg-slate-950">
              MXN
            </option>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <input
            ...
            onChange={(e) =>
              onExpenseDraftChange((current) => ({ ...current, exchangeRate: e.target.value }))
            }
          ...
          <select
            ...
            onChange={(e) =>
              onExpenseDraftChange((current) => ({ ...current, paidBy: e.target.value }))
            }
            className="rounded-2xl border border-white/10 bg-black/10 px-4 py-3 text-sm text-white outline-none"
          >
        ...
        <input
          ...
          onChange={(e) =>
            onExpenseDraftChange((current) => ({ ...current, expenseDate: e.target.value }))
          }
          className="rounded-2xl border border-white/10 bg-black/10 px-4 py-3 text-sm text-white outline-none"
        />
        <textarea
          ...
          onChange={(e) =>
            onExpenseDraftChange((current) => ({ ...current, notes: e.target.value }))
          }
          placeholder="Notas"
          rows={3}
          className="rounded-2xl border border-white/10 bg-black/10 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30"
        ...
        <div className="rounded-2xl border border-white/8 bg-black/10 p-4">
              ...
              return (
                ...
                >
                  <input
                    ...
                    checked={checked}
                    onChange={() =>
                      onSelectedParticipantsChange((current) =>
                        checked
          ...
          <div className="mt-3 text-xs text-white/45">
            Si están todos seleccionados, se reparte entre todos.
          </div>
        </div>
        <button
          disabled={saving || !participants.length}
          className="inline-flex items-center justify-center gap-2 rounded-2xl bg-amber-300 px-4 py-3 text-sm font-medium text-black transition hover:bg-amber-200 disabled:opacity-60"
        >
