    ...
    <Surface className="p-6">
      <div className="flex items-center justify-between gap-3">
        ...
        {!participants.length && participantPresets.length ? (
          <button
            onClick={onEnsurePresetParticipants}
            className="rounded-full border border-white/10 px-4 py-2 text-sm text-white/80 hover:border-amber-300/40 hover:text-white"
          >
            Cargar base
          </button>
        ) : null}
      ...
      <div className="mt-4 space-y-3">
        {participants.map((participant) => (
          <div
            key={participant.id}
            className="rounded-2xl border border-white/8 bg-white/4 px-4 py-3 text-sm text-white/80"
          >
      ...
      <form onSubmit={onAddParticipant} className="mt-5 grid gap-3">
        <input
          ...
          onChange={(e) =>
            onParticipantDraftChange((current) => ({ ...current, name: e.target.value }))
          }
          placeholder="Nombre"
        ...
        />
        <input
          value={participantDraft.emoji}
          onChange={(e) =>
            onParticipantDraftChange((current) => ({ ...current, emoji: e.target.value }))
          }
