    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <Surface className="p-5">
        <div className="flex items-center gap-2 text-sm font-medium text-white">
          <Wallet className="h-4 w-4 text-amber-300" /> Resumen de gastos
        </div>
        <div className="mt-4 text-3xl font-semibold text-white">
          {money(totalGeneral, baseCurrency)}
        </div>
      ...
      <Surface className="p-5 lg:col-span-2">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 text-sm font-medium text-white">
              <Calculator className="h-4 w-4 text-amber-300" /> Balance y liquidación
            </div>
            <div className="mt-1 text-sm text-white/55">
              Quién pagó de más y quién le debe a quién.
            </div>
          </div>
          {!shareMode ? (
            <button
              onClick={onGenerateShareLink}
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white transition hover:border-amber-300/40 hover:text-amber-200"
            >
        ...
        <div className="mt-4 space-y-2 text-sm text-white/70">
          {settlements.length ? (
            settlements.map((settlement) => (
              ...
              >
                <span className="font-medium text-white">{settlement.from}</span> le debe{' '}
                <span className="font-medium text-white">
                  {money(settlement.amount, baseCurrency)}
                </span>{' '}
                a <span className="font-medium text-white">{settlement.to}</span>
              </div>
            ))
          ) : (
            <div className="rounded-2xl border border-white/8 bg-white/4 px-4 py-3">
              Sin saldos pendientes por ahora.
            </div>
          )}
        </div>
      </Surface>
