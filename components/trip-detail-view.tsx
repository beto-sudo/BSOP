...
export function TripDetailView({ trip, shareMode = false }: { trip: TravelTrip; shareMode?: boolean }) {
      ...
      <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
          ...
          <Surface className="p-6">
            ...
            <div className="space-y-4">
              {trip.itinerary.map((item) => (
                <details key={item.day} open className="group rounded-3xl border border-white/8 bg-white/4 p-5">
                  <summary className="cursor-pointer list-none">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                      <div>
                        ...
                        {item.focus ? <div className="mt-2 text-sm text-white/50">Objetivo: {item.focus}</div> : null}
                      </div>
                      <div className="text-xs uppercase tracking-[0.24em] text-white/35 group-open:text-amber-300">Expandir / colapsar</div>
                    </div>
                  </summary>
                  ...
                  {item.tips?.length ? (
                    <div className="mt-4 rounded-2xl border border-amber-300/15 bg-amber-300/5 p-4">
                      <div className="flex items-center gap-2 text-sm font-medium text-white"><CheckCircle2 className="h-4 w-4 text-amber-300" /> Tips operativos</div>
                      <ul className="mt-3 space-y-2 text-sm text-white/65">
                        {item.tips.map((tip) => <li key={tip} className="flex gap-2"><span className="mt-1 text-amber-300">•</span><span>{tip}</span></li>)}
                      </ul>
                    </div>
                  ) : null}
                </details>
          ...
          <Surface className="p-6">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-2xl border border-white/8 bg-white/4 px-4 py-3"><div className="text-xs uppercase tracking-[0.24em] text-white/40">{label}</div><div className="mt-2 text-lg font-semibold text-white">{value}</div></div>
              ))}
            </div>
          </Surface>
        ...
        <div className="space-y-6">
          {trip.accommodation ? (
            <Surface className="p-6">
              ...
              <div className="space-y-4 text-sm text-white/70">
                <div><div className="text-white/40">Propiedad</div><div className="mt-1 text-base font-medium text-white">{trip.accommodation.name}</div></div>
                <div><div className="text-white/40">Dirección</div><div className="mt-1 text-white">{trip.accommodation.address}</div></div>
                {trip.accommodation.checkIn ? <div><div className="text-white/40">Check-in</div><div className="mt-1 text-white">{trip.accommodation.checkIn}</div></div> : null}
                {trip.accommodation.checkOut ? <div><div className="text-white/40">Check-out</div><div className="mt-1 text-white">{trip.accommodation.checkOut}</div></div> : null}
                {trip.accommodation.checkInMethod ? <div><div className="text-white/40">Método</div><div className="mt-1 text-white">{trip.accommodation.checkInMethod}</div></div> : null}
                {trip.accommodation.confirmationCode ? <div><div className="text-white/40">Confirmación</div><div className="mt-1 text-white">{trip.accommodation.confirmationCode}</div></div> : null}
                {trip.accommodation.host ? <div><div className="text-white/40">Host</div><div className="mt-1 text-white">{trip.accommodation.host}</div></div> : null}
                {trip.accommodation.totalPaid ? <div><div className="text-white/40">Pagado</div><div className="mt-1 text-white">{trip.accommodation.totalPaid}</div></div> : null}
              </div>
            ...
            <Surface className="p-6">
              ...
              <div className="space-y-3">
                {trip.accommodations.map((item) => (
                  <div key={item.label + item.value} className="rounded-2xl border border-white/8 bg-white/4 p-4">
          ...
          {trip.pickupPoints?.length ? (
            <Surface className="p-6">
              <div className="mb-5 flex items-center gap-3 text-white"><MapPin className="h-5 w-5 text-amber-300" /><h2 className="text-lg font-semibold">Puntos clave de ruta</h2></div>
              <div className="space-y-3">
                {trip.pickupPoints.map((point) => (
                  <div key={point.name + point.timing} className="rounded-2xl border border-white/8 bg-white/4 p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {point.mapsHref ? <a href={point.mapsHref} target="_blank" rel="noreferrer" className="inline-flex shrink-0 items-center gap-1 rounded-full border border-white/10 px-3 py-2 text-xs text-white/70 transition hover:border-amber-300/40 hover:text-white">Maps <ArrowUpRight className="h-3.5 w-3.5" /></a> : null}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Surface className="p-6">
            <div className="mb-5 flex items-center gap-3 text-white"><CheckCircle2 className="h-5 w-5 text-amber-300" /><h2 className="text-lg font-semibold">Condiciones y notas</h2></div>
            <div className="rounded-3xl border border-white/8 bg-white/4 p-5">
              <p className="text-sm leading-7 text-white/65">{trip.conditions.summary}</p>
              <ul className="mt-4 space-y-3 text-sm text-white/60">
                {trip.conditions.bullets.map((bullet) => <li key={bullet} className="flex gap-2"><span className="mt-1 text-amber-300">•</span><span>{bullet}</span></li>)}
              </ul>
            </div>
            <div className="mt-4 rounded-3xl border border-amber-300/20 bg-amber-300/5 p-5">
              ...
              <ul className="mt-4 space-y-3 text-sm text-white/65">
                {trip.conditions.warnings.map((warning) => <li key={warning} className="flex gap-2"><span className="mt-1 text-amber-300">•</span><span>{warning}</span></li>)}
              </ul>
            </div>
          </Surface>

          <Surface className="p-6">
            <div className="mb-5 flex items-center gap-3 text-white"><Link2 className="h-5 w-5 text-amber-300" /><h2 className="text-lg font-semibold">Links clave</h2></div>
            <div className="space-y-3">
              {trip.keyLinks.map((link) => (
                <a key={link.label + link.note} href={link.href} target="_blank" rel="noreferrer" className="block rounded-2xl border border-white/8 bg-white/4 p-4 transition hover:border-amber-300/30 hover:bg-white/6">
                  <div className="flex items-center justify-between gap-3">
                    <div><div className="text-sm font-medium text-white">{link.label}</div>{link.note ? <div className="mt-1 text-xs text-white/50">{link.note}</div> : null}</div>
          ...
          <Surface className="p-6">
            ...
            <ul className="space-y-3 text-sm text-white/70">
              {trip.todo.map((item) => <li key={item} className="flex gap-3 rounded-2xl border border-white/8 bg-white/4 px-4 py-3"><span className="mt-0.5 text-amber-300">□</span><span>{item}</span></li>)}
            </ul>
          </Surface>
        </div>
      </div>
