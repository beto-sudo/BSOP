...
**Estado:** planned
**Próximo hito:** Beto confirma los nombres canónicos (checklist + recepción) → arrancar Sprint 1: S1a (rename de nombres, sin tocar %), S1b (objeto `recepcion_obra` + checklist de verificaciones) y S1c (gate `dilesa.construccion.recepcion`). La redistribución de % queda diferida a arranques nuevos (no se toca con obras en construcción).
**Dueño:** Beto
**Creada:** 2026-06-15
...
- [ ] **Sprint 1 — Recepción de obra al contratista (el gap real).** Tres entregables:
  - **S1a · Estandarizar SOLO el nombre** de la tarea de checklist y de la tarea de recepción en los 14 prototipos (UPDATE de texto en `tareas_construccion`, sin tocar `porcentaje_costo` ni `plantilla_tareas`). Nombres canónicos **propuestos** (pendiente confirmar con Beto): checklist = `Checklist de Recepción a Contratista`; recepción = `Recepción de Vivienda Terminada`. Seguro para obras en curso (el avance no depende del nombre). NO se redistribuyen porcentajes (ver D2 + Deuda diferida).
...
- **Tocar % rompería el avance de obras en curso.** Por eso v1 NO redistribuye porcentajes (D2): solo renombra. La estandarización de pesos queda diferida a arranques nuevos.
...
- **2026-06-15 (revisión D2)** — Beto: **NO tocar porcentajes ni conceptos mientras haya obras en construcción** (rompería el avance a media obra). La estandarización de % se difiere a **arranques nuevos** (LDS ya cerró arranques; LDLE solo a futuro). v1 hace **solo rename** del nombre de las tareas de checklist y recepción en todos los prototipos (sin tocar %). Las 2 familias conviven con pesos dispares hasta el rebalanceo futuro (ver Deuda diferida).
