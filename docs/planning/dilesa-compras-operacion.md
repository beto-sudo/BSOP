...
**Estado:** in_progress
**Próximo hito:** **Sprint 5c (folio secuencial RFQ-2026-NNNN) en revisión de Beto** — trigger ya aplicado a prod; con esto el **pulido del flujo queda completo** (5a/5b/5c + data-fix). (Pendiente de Beto aparte: Sprint 3b — cuadro comparativo de RFQ + envío masivo; Sprint 4 — evento de recepción.) Sprints 5a/5b mergeados.
**Dueño:** Beto
**Creada:** 2026-06-24
**Última actualización:** 2026-06-25 (Sprint 5c: folio secuencial RFQ-2026-NNNN vía trigger `erp.fn_cotizacion_asignar_folio`, aplicado a prod; Sprints 5a/5b + data-fix mergeados/hechos)
...
- **2026-06-25 (Pulido del flujo) — Una cotización (RFQ) por requisición.** Beto: pedir cotizaciones sobre una requisición que ya tiene una RFQ activa abre ESA (no crea otra); para comparar más proveedores se agregan a la misma. Resuelve el enredo del caso REQ-MQSAD18S (3 RFQs sueltas para una requisición).
- **2026-06-25 (Pulido del flujo) — Adjudicación reversible + candados de cancelación.** Cancelar la OC nacida de una RFQ reabre la RFQ (vuelve a comparable); cancelar una requisición con cadena viva (RFQ adjudicada / OC viva) se bloquea. Hoy cancelar la OC deja la RFQ `adjudicada` huérfana (caso REQ-MQSAD18S).
...
**Sprint 5 — Pulido del flujo RFQ/cancelación (gobierno; lógica + algo de schema).** Detonante: feedback de Beto sobre el caso REQ-MQSAD18S (requisición con 3 RFQs, una adjudicada con OC cancelada, y el resto huérfanas/abiertas).

- **5a (sin schema):** candado de **una RFQ por requisición** (pedir cotización abre la existente en vez de crear otra); bloquear cancelar una requisición con cadena viva; quitar el "sin autorizar" del hilo del gasto (drift: la autorización de requisición ya no existe).
- **5b (lógica):** **adjudicación reversible** — cancelar la OC nacida de una RFQ reabre la RFQ a `comparada`.
- **5c (schema):** folio secuencial de RFQ (`RFQ-{año}-{NNNN}`, mismo patrón de trigger que la OC).
- **Data-fix (con OK de Beto):** ordenar el caso REQ-MQSAD18S (cerrar RFQs huérfanas/duplicadas).
...
- **2026-06-24 — Sprint 3a — enviar la OC por email + visibilidad** ([#1021](https://github.com/beto-sudo/BSOP/pull/1021), sin auto-merge — UI + correo real). `POST …/ordenes-compra/[id]/pdf` manda la OC en PDF por email (Resend, fail-open en el catálogo + log en `notification_log`); acción reintentable con confirmación que **no cambia el estado** (lo gobierna "Marcar enviada") para evitar inconsistencias por fallo parcial. **Atiende el feedback de Beto** ("no veo dónde imprimir"): "Imprimir OC" y "Enviar al proveedor" pasan al menú ⋯ de la fila (antes solo en el footer del drawer). Sin migración (el kill-switch/config del correo, slug `dilesa_orden_compra`, queda como INSERT opcional posterior). **Mergeado** (#1021).
- **2026-06-25 — Sprint 5a (pulido del flujo) — candado de una RFQ por requisición** ([#1035](https://github.com/beto-sudo/BSOP/pull/1035), mergeado). `pedirCotizaciones` verifica si la requisición ya tiene una cotización activa (no cancelada) y abre ESA en vez de crear otra — resuelve el enredo del caso REQ-MQSAD18S (3 RFQs sueltas, una adjudicada con OC cancelada). Detonante: Beto reportó la confusión del proceso. Sin schema.
- **2026-06-25 — Sprint 5b (pulido del flujo) — adjudicación reversible + cascade** (sin auto-merge). Cancelar la OC nacida de una RFQ reabre la cotización a `comparada` (y sus proveedores `elegida`/`descartada` → `respondida`) para re-adjudicar o cerrar — evita la RFQ `adjudicada` huérfana del caso REQ-MQSAD18S. Cancelar una requisición cancela en cascada sus cotizaciones en curso (`abierta`/`comparada`); las adjudicadas las gobierna su OC (el gate `con_oc` ya impide cancelar la requisición). Lógica cliente, sin schema. Sigue: 5c folio secuencial de RFQ.
- **2026-06-25 — Data-fix REQ-MQSAD18S** (prod, OK de Beto: «reabrir la adjudicada»). El caso tenía 3 cotizaciones: **RFQ-MQSADDFP** `adjudicada` con su OC (OC-MQSH3X52) cancelada → reabierta a `comparada` (re-adjudicable/cerrable; su proveedor ya estaba `respondida`); **RFQ-MQTNWS6A** y **RFQ-MQTW2ETE** `abierta` vacías (0 proveedores, duplicados del bug que tapó 5a) → `cancelada`. Vía MCP `execute_sql` (UPDATE de datos en una query atómica con guardas por estado; no toca el ledger de migraciones). Verificado: 1 RFQ comparable + 2 canceladas.
- **2026-06-25 — Sprint 5c (pulido del flujo) — folio secuencial de RFQ** (sin auto-merge). Trigger `erp.fn_cotizacion_asignar_folio` (BEFORE INSERT en `erp.cotizaciones`, gemelo del de OC) asigna `RFQ-{año}-{NNNN}` por (empresa, año) con advisory lock (salt `rfq` para no contender con OC); los 2 productores (`requisiciones` + `cotizaciones` modules) dejan de mandar `codigo` desde el cliente y leen el folio de vuelta para el aviso. Migración `20260625203023` aplicada a prod por MCP + ledger reconciliado (`repair applied/reverted`, 1:1 sin drift) + SCHEMA_REF regenerado. Las RFQ viejas (`RFQ-<base36>`) no colisionan (no matchean `RFQ-{año}-%`). Con esto el **pulido del flujo queda completo**: candado (5a) + reversibilidad/cascade (5b) + folio legible (5c) + caso viejo ordenado.
