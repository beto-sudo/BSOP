...
**Estado:** done
**Próximo hito:** — (iniciativa completa; Sprints 1-3 en prod)
**Dueño:** Beto
**Creada:** 2026-06-15
**Última actualización:** 2026-06-16 (Sprint 3 — tope estricto `nativa-sin-promo → 0` — entregado; iniciativa cerrada)
...
- **2026-06-16** — **Sprint 3 entregado · iniciativa cerrada.** Beto confirmó que el catálogo opera con la única promo existente (LDLE-ISC) y autorizó arrancar. Blast radius verificado en prod = **0** (4 ventas nativas, ninguna con descuento → ninguna venta existente cambia; es puramente forward-looking). Helper `topeDescuentoAutorizado(promoMonto, esLegacy)` en el motor + usado en los 3 callers (`cuadratura-server`, `use-venta-resumen`, detalle): con promo → su monto; nativa de BSOP sin promo → **0** (descuento solo vía catálogo); legacy de Coda sin promo → null (sin tope). Tests del helper + del saldo con tope 0. Code-only (sin migración).
- **2026-06-16** — **Sprint 2 entregado.** Migración `20260616023142_dilesa_modulo_ventas_promociones` aplicada a prod (sub-slug `dilesa.ventas.promociones` + backfill de permisos clonados del padre — 8 roles). Tab "Promociones" en el hub de Ventas (`app/dilesa/ventas/promociones/page.tsx`): lista + alta/edición de monto (tope), prototipos aplicables, vigencia, activa; WRITE solo Dirección/admin. RBAC sync en `layout.tsx` (TABS), `lib/permissions.ts` (ROUTE_TO_MODULE), `lib/permissions.test.ts` + `lib/permissions-deps.ts`. Auto-asignación de la promo aplicable en la captura de venta (badge "auto-asignada"). 6 checks verdes.
