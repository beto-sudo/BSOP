...
**Empresas:** DILESA
**Schemas afectados:** `dilesa` (`ventas` — columnas de descuento + `promocion_id`; `promociones` — CRUD nuevo; RPC nueva `fn_actualizar_descuentos_venta` + helper de auditoría), `core` (`audit_log`), `erp` (lectura). Mayormente código: `lib/dilesa/cuadratura*.ts`, `components/dilesa/cuadratura-ajustes.tsx`, `app/dilesa/ventas/{nueva,[id]}`, módulo nuevo `app/dilesa/promociones`.
**Estado:** planned
**Próximo hito:** Arrancar Sprint 1 — RPC `fn_actualizar_descuentos_venta` (audita anterior/nuevo a `core.audit_log` + garantiza `sum(buckets)=descuento_total`) + motor lee `descuento_total` como autoritativo. Migración la aplica Beto.
**Dueño:** Beto
**Creada:** 2026-06-15
**Última actualización:** 2026-06-15 (promovida desde el diagnóstico del tope de descuento en la Cuadratura — ver `## Bitácora`)
