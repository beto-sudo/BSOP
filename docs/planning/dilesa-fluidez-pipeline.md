...
**Estado:** in_progress
**Próximo hito:** Pulido post-v1: PDF del radar, segmentación por crédito/proyecto, metas por tipo de crédito, y **confirmar con Beto la clasificación interna/tercero** de `responsableFase`. El arco v1 (S1 días → S2 radar+banda → S3 metas) quedó completo.
**Dueño:** Beto
**Creada:** 2026-06-28
**Última actualización:** 2026-06-30 (polish: pestaña Fluidez por venta + optimización de carga lista/radar)
...
- **2026-06-29** — **S2a entregado: el radar de cuellos por fase.** Reporte «Calificación por fase» (`/dilesa/ventas/reportes/calificacion-por-fase`): por fase del pipeline (1–14), mediana/p90/n de días con corte temporal (mes/trim/sem/año/todo/solo-activas), banda vs. su histórico (verde/ámbar/rojo, n<5=gris), tendencia Δ vs. periodo anterior, KPIs (cuello por p90, fases lentas). Capa de datos (migración `20260629232520`): `v_venta_fase_duraciones` (duración limpia por tramo con flags retroceso/abierto), `v_fase_benchmark` (mediana/p90 histórico, fases 1–14), RPC `fn_fase_calificacion(empresa,desde,hasta)` (corte por periodo sin traer 14k filas). Limpieza validada en prod: excluye negativos (orden posición≠fecha) y fases 15–17 (Entregada/Conformidad contaminadas por sellado en bloque de migración). Clasificación interna/tercero en `fases.ts` (`responsableFase`, propuesta a confirmar con Beto — informativa en S2a, gobierna el score en S2b). Motor puro `lib/dilesa/reportes/calificacion-por-fase.ts` + tests. Hallazgo inmediato (último trimestre): **Formalizada mediana 12.5 d vs 4 histórico (×3, p90 102 d)** = cuello real. R4 aplicado: bandas, sin score 0-100 sofisticado. PDF y segmentación crédito/proyecto diferidos a S2a.2.
- **2026-06-29** — **S2b entregado: banda de fluidez por venta.** Los "días en fase" de S1 ahora se colorean por el **benchmark de su fase** (S2a), no por umbral fijo: el mismo "18 d" significa "lento PARA ESTA FASE" (verde ≤ mediana · ámbar ≤ p90 · rojo > p90 = más lento que el 90% de los casos). En la lista: columna **Fluidez** ordenable (Al día/Lenta/Crítica, críticas arriba) + tooltip que explica el número ("18 d en Formalizada · típico: mediana 4, p90 102"). En el expediente: header + bloque Comercial recoloreados igual. Helper puro `lib/dilesa/fluidez-venta.ts` (banda = riesgo actual de la fase en curso, R4) + tests. **Sin migración**: consume `v_fase_benchmark` de S2a (auto-merge). Fallback a umbral fijo de S1 cuando no hay benchmark (fases 15–17 / n bajo).
- **2026-06-30** — **S3 entregado: metas editables por fase (v1 completo).** Dirección fija una **meta** en días por fase; donde haya, sustituye a la mediana histórica como **vara** del semáforo (radar) y del color de fluidez por venta (lista/expediente). Tabla `dilesa.fase_metas` (migración `20260630003957`): RLS lee cualquiera de la empresa, **escribe solo Dirección/admin** vía `erp.fn_es_direccion` (gate en DB, sin server action — espejo del presupuesto). Vista `dilesa.v_fase_vara` resuelve el `COALESCE(meta, mediana)` en un solo lugar; la consumen el radar y el per-venta. Radar: columna **Meta** editable (input por fila para Dirección, "auto" para los demás) con upsert directo + re-fetch; la banda y la columna Histórico conviven (la meta no borra el histórico de referencia). Motor extendido (`meta`/`vara` en `FaseBenchmark`/`FaseCalificacionRow`) + test. Resuelve M1 del adversarial ("benchmark que se mueve solo"): la meta da el norte absoluto.
- **2026-06-30** — **Polish (feedback de Beto): pestaña Fluidez + performance.** (1) **Pestaña «Fluidez»** en el expediente (`/dilesa/ventas/[id]/fluidez`): cómo va la venta contra el objetivo fase por fase — días en cada fase recorrida vs. la vara, banda, fase en curso resaltada, resumen (fase actual + banda, fases en objetivo, críticas). Reusa el slug RBAC `dilesa.ventas.pipeline` (sin sub-slug nuevo → sin migración). Helper puro `lib/dilesa/fluidez-venta-detalle.ts` + tests. (2) **Optimización de carga (dos causas).** Primero se paralelizaron los round-trips de la lista (7 queries en serie → 1 `Promise.all`) y del radar. Pero el problema de fondo era otro: midiendo con `EXPLAIN ANALYZE` **bajo el rol `authenticated` con RLS forzada** (el primer EXPLAIN engañó por correr como superusuario, RLS off), el costo saltó de 2,637 a 14,316 — las vistas `security_invoker` evalúan la **RLS por fila** (`fn_has_empresa` + `fn_es_vendedor_restringido`, que consultan tablas core) sobre ~15k filas de `venta_fases` → segundos / "statement timeout" en semestre. **Fix** (migración `20260630022903`): exponer las lecturas pesadas como funciones **`SECURITY DEFINER`** (`fn_fase_vara`, `fn_ventas_lista_antiguedad`, y `fn_fase_calificacion` pasa de INVOKER a DEFINER) que autorizan la empresa **una vez** (guard `fn_has_empresa(p_empresa)` arriba) y escanean como owner (RLS off) filtrando por `p_empresa` → vuelve a ~30ms. Solo exponen tiempos agregados/por-tramo, no info financiera. Memoria [[reference_rls_fn_has_empresa_per_row]].
