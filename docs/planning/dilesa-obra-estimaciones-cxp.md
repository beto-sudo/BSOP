...
**Estado:** in_progress
**Próximo hito:** Sprint 3 **en prod** (PR #1091 mergeado, migración `20260627010241` aplicada + ledger reconciliado). Arranca el **Sprint 4** — retención acumulada (fondo de garantía por contrato) + liberación guiada en la estimación de finiquito.
**Dueño:** Beto
**Creada:** 2026-06-26
...
| 3   | **Amortización automática del anticipo.** Modelar el anticipo como saldo a recuperar; descontar la proporción (`anticipo_pct`) en cada estimación de avance al emitir a CxP; mostrar "anticipo por amortizar" en el estado de cuenta del contrato.                                                                                                                           | ✅ prod (#1091) |
...
- **2026-06-26 — Sprint 3 construido (PR abierto, sin auto-merge).** Amortización lineal del anticipo (decisión Beto: contratista factura el avance NETO).
...
- **2026-06-26 — Sprint 3 en prod (PR #1091 mergeado).** Beto dio OK. Episodio multi-sesión: el `schema:check` de CI rompió por **drift heredado** (prod adelantado a main: FK `usuario_id` + tablas `rdb.*` eliminadas) — 3 sesiones aplicando migraciones por MCP antes de mergear dejaron huérfanos _remote-only_. Mi sesión pausó (PR a draft, S3 sin aplicar — la "limpia") y fue al final. La iniciativa `derivados-sin-drift` (#1093) creó archivos `reconciliacion_*` que emparejan los huérfanos en el ledger. Tras mergear main: `schema:check` 1:1, apliqué la migración `20260627010241` por MCP, reconcilié mi huérfano (`…020225` reverted), regeneré SCHEMA_REF (solo `amortizacion_aplicada`) + types. CI verde, auto-merge. Lección en memoria `reference_dilesa_estimaciones_destajo_cxp`. **Siguiente: Sprint 4** (retención + finiquito).
