...
**Estado:** in_progress
**Próximo hito:** iniciativa COMPLETA en alcance comprometido — pendientes operativos: piloto de S5 (relotificación) + recopilar KMZ/escrituras con el checklist
**Dueño:** Beto
**Creada:** 2026-07-01
**Última actualización:** 2026-07-02 (S1-S8 en prod — alcance completo)
...
- **2026-07-02 — S7 Ficha comercial PDF + email:** ruta `/api/dilesa/portafolio/activo/[id]/ficha` (react-pdf, branding DILESA HeaderBand/FooterBand): datos generales, características por tipo (whitelist COMERCIAL — notas internas/embudo/bitácora jamás salen), hasta 2 fotos del expediente, precio de lista si destino cuenta_venta, link a Google Maps. Botones "Ficha PDF" (todos) y "Enviar ficha" (Dirección/admin) en el expediente; el envío = dialog con destinatarios/asunto/mensaje editables + preview del PDF + confirmación explícita → Resend con adjunto base64, catálogo `dilesa_ficha_comercial` (kill switch/from/extra/log, seed en migración) y reply-to del operador.
- **2026-07-02 — S8 Matching asistido de escrituras:** botón "Ligar escrituras" en el Inventario (Dirección/admin). Dos pasos: heurística determinista (`activos.numero_escritura` == `documentos.numero_documento` normalizado → confianza alta) + IA vía `lib/ai` (uso `dilesa-matching-escrituras`, registrado en el registry ADR-046) que cruza el texto/meta de las 58 escrituras contra el catálogo de predios (parcelas, ranchos, superficies con conversión ha→m², claves) y propone pares con confianza y razón citando la evidencia. NADA se liga solo: confirmación una por una (usa `ligarDocumentoActivo`, misma tubería del ligado manual); ids alucinados se descartan; IA best-effort (sin API key caen solo las heurísticas).
- **2026-07-02 — Inventario limpio (feedback de Beto al estrenar):** el tab Inventario excluye `prospecto`/`descartado` (no son patrimonio — viven en Evaluación hasta adquirirse) y `tipo=cara` (las 52 caras se ven en el expediente de su estructura; arrendamiento las sigue consumiendo igual). KPI "En evaluación" → "Superficie". El prop `vista` de `PortafolioModule` se retiró (muerto desde que S6 le dio módulo propio a Evaluación).
- **2026-07-02 — Espacios publicitarios + municipios completos + exportes (feedback Beto):** tipo `espacio_publicitario` con `subtipo` (espectacular/unipolar/padel/valla/otro) absorbe a espectacular y unipolar — `activo_espectacular` renombrada a `activo_espacio_publicitario`, las 26 estructuras migradas (10 padel por nombre), caras intactas (arrendamiento no se toca), `activo_unipolar` deprecada sin drop, `fn_alta_activo`/`_activo_upsert_satelite` redefinidas (el branch de lote ahora preserva calle/esquina/frente verde al editar — gap del upsert viejo). Data-fix: espacios publicitarios y caras → municipio Piedras Negras; `estado_geo='Coahuila'` donde municipio ∈ {P. Negras, Nava}; prospectos quedan vacíos a propósito (decisión Beto — se capturan al evaluar). Exportes del inventario: botón **PDF** (listado landscape con filtros aplicados, react-pdf) y **CSV** (client-side con BOM para Excel).
