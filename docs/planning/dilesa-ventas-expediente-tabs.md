...
**Dueño:** Beto
**Creada:** 2026-06-18
**Última actualización:** 2026-06-18 (pulido post-cierre #957: captura igualada al expediente — ficha al ancho 6xl + tabs persistentes debajo. Antes: 3 sprints #948/#949/#950 en prod)
...
- **2026-06-18** — **Pulido post-cierre = el "Sprint 3 diferido" (consistencia captura↔expediente).** Beto reportó que al capturar una fase la ficha se veía más chica y los tabs salían arriba del resumen (vs. debajo en el detalle) — justo el item que el Sprint 2 había diferido. Raíz: la rama captura del layout pintaba `VentaExpedienteTabs` en `max-w-3xl` **encima** del `CapturarFaseHeader` (que traía su propia ficha angosta), mientras el expediente vive en `max-w-6xl` con la Zona A en `VentaExpedienteShell`. Fix: nuevo `components/dilesa/venta-detalle/captura-shell.tsx` (`VentaCapturaShell` + contexto `useVentaCapturaResumen`) que monta la **misma Zona A persistente** (back-link + `OperacionResumen` + tabs, `max-w-6xl`, carga única → sin parpadeo) por encima del formulario; `CapturarFaseHeader` reducido al título de fase; `VentaExpedienteTabs` extraído a `tabs.tsx` (compartido sin arrastrar el provider); ancho de las 16 páginas de captura `3xl→6xl` (decisión Beto: todo al ancho del detalle); F13/F10 leen el resumen del contexto (sin doble carga) y se eliminó el fetch de cliente/identificador ya redundante en 14 páginas (−726 líneas netas). Sin migración (puro UI); 6 checks verdes. PR [#957](https://github.com/beto-sudo/BSOP/pull/957) — **sin auto-merge** (UI revisable en preview).
