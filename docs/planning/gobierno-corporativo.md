...
- [ ] **Sprint 3 — Tab "Actas de asamblea"**:
  ...
  - Route handlers `/api/empresas/[id]/actas` (+ sub-recursos) con `requireAdmin`. Tests.
- [ ] **Sprint 4 — Seed DILESA + closeout**:
  - Sembrar datos reales de DILESA leyendo el Reglamento de Gobierno (ago-2021): 3 familias al 33.33%, mayorías por tipo de decisión, derecho del tanto, consejo y mandatos. Subir el reglamento PDF a `erp.documentos` y ligarlo.
  - Actualizar `ARCHITECTURE.md` §2 (nuevas tablas core) si aplica. Closeout + barrido de Reminders.
