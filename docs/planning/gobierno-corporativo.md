...
- **Sprint 4 — Seed DILESA + closeout**:
  ...
  - [ ] **4b · Subir PDFs** — Reglamento + 35 actas a Storage (`adjuntos`) + `erp.documentos` + ligar (`gobierno_actas.documento_id` / `gobierno_config.reglamento_documento_id`), casando por fecha (el numerado del folder no siempre cuadra con el índice).
  - [ ] **4c · Capturar acuerdos/votos** — leer cada acta y sembrar `gobierno_acta_acuerdos` + `gobierno_acta_votos` (voto por socio) + `gobierno_acta_asistentes`. Por lotes.
  - [ ] **Closeout** — `ARCHITECTURE.md` §2 (8 tablas `core` nuevas) + barrido de Reminders + verificación de Beto (día del reglamento, consejo actual + voto CHC).
