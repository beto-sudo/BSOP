...
- **2026-05-19** — La automatización del CSV usa **login email+password en cada corrida**, no un refresh_token guardado. Razón: el refresh_token de Playtomic vive ~60 días; guardarlo haría que el cron se rompa en silencio cada ~2 meses (failure mode operativo malo). Login fresco por corrida es self-sustaining — sólo se rompe si Beto cambia su password de Playtomic. Trade-off aceptado: se almacena el password real de Playtomic Manager (no hay service-account; el third-party API no expone payment_method, que es justo la razón de existir de esto). Mitigación: protocolo Beto-first (1Password → Vercel env), nunca en chat/commits/docs; el código nunca loguea password ni tokens.
- **2026-05-19** — Cron diario (no semanal) con ventana rodante de 60 días + upsert idempotente por `payment_id`. Razón: los pagos en club se marcan PAID en Playtomic días/semanas después del juego; una ventana corta los perdería. Re-importar un periodo solapado es seguro (PK = `payment_id`). Diario da datos frescos a `/conciliacion` sin costo (idempotente, indexado).
...
- **2026-05-11** — **Veredicto de Playtomic sobre automatización vía API.** Beto escribió a `support@playtomic.com` (rebotó — dominio no acepta SMTP). Se localizó el contacto real vía Gmail: Account Manager Juan Carlos Alaguich (`juancarlosalaguich@playtomic.io`) reenvió la consulta al equipo de API. Respuesta: (1) el API third-party expone el `origin` de la **reserva** (`APP_IOS`/`CLUB`/`WEB` — que ya teníamos en `raw_json.origin`), NO el `origin`/`payment_method` del **pago**; (2) `payment_method` NO está en el API y no hay fecha para exponerlo (Wellhub/Club wallet/Free payment siguen CSV-only); (3) opciones webhook + endpoint de detalle escaladas a product, sin compromiso. **Conclusión: automatización vía API no viable corto plazo.** Canal técnico correcto registrado: `americasupport@playtomic.io` / WhatsApp clubes +1 949 490 6280. El CSV manual semanal sigue siendo load-bearing; la palanca que controlamos es la auto-conciliación por heurística de notas. Próximo paso aprobado por Beto: investigar automatización del **download del CSV vía web** (panel manager.playtomic.com) ya que no hay API.

- **2026-05-19** — **Automatización del download del CSV vía web (cron Vercel).** Branch `feat/rdb-playtomic-csv-cron`. Se mapeó el flujo web del Manager desde capturas del navegador: `POST /api/v3/auth/login` (email+password → refresh token) → `POST /api/v3/auth/token` (canje a scope `ROLE_TENANT_MANAGER`) → `GET /api/v1/club_payments/export` (CSV, mismo formato que ya parsea `lib/playtomic/csv-import.ts`). El endpoint de export se verificó verbatim de la captura; el flujo de auth es defensivo (acepta snake/camel case, falla ruidoso). Implementado: helper compartido `lib/playtomic/payments-import-upsert.ts` (extraído del action de upload manual, ahora usado por ambos call sites sin divergencia de lógica), cliente `lib/playtomic/manager-api.ts`, cron route `app/api/cron/playtomic-csv-import/route.ts` (guard `CRON_SECRET`, ventana rodante 60d, idempotente, override `?lookbackDays=` para backfill puntual), entrada en `vercel.json` (`0 9 * * *` = 03:00 CST), env vars documentadas en `.env.local.example`. 8 tests nuevos del cron (auth gate + happy path + clamps + error sin filtrar secretos); 716/716 verdes. **Pendiente operativo (Beto-first):** Beto carga `PLAYTOMIC_MANAGER_EMAIL` / `PLAYTOMIC_MANAGER_PASSWORD` en 1Password → Vercel env (scope Production) para activar el cron; primer run real valida el flujo de auth end-to-end.

- **2026-05-20** — **Cron CSV live en prod.** PR #470 mergeado (commit d66df86). Beto cargó credenciales Playtomic en 1P (`PLAYTOMIC_MANAGER_EMAIL` / `PLAYTOMIC_MANAGER_PASSWORD`, field `credential`); sincronizadas a Vercel env Production vía `op read | vercel env add`, valor nunca expuesto. Pre-merge: validación standalone con `op + npx tsx` levantó 200 filas/7d con todos los `payment_method` esperados (Cash 66, Credit card 87, Apple Pay 13, Free payment 4, Club wallet 12, Waitry 1, Wellhub 1, Crédito Interno 16). Post-merge: primer trigger prod via `curl -H "Authorization: Bearer $CRON_SECRET"` → HTTP 200 en 4s, 1363 filas (27 inserted + 1336 updated, upsert idempotente), `payment_date_max=2026-05-20T01:59Z`. Auth flow funcionó al primer intento con el parseo defensivo. El upload manual semanal queda **operativamente retirado** (cron diario 03:00 CST asume la función). Memoria `reference_playtomic_soporte_api.md` actualizada.
