...
**Estado:** in_progress
**Próximo hito:** S3 — **13 superficies DILESA cubiertas** (smokes read-only de Compras·5, Construcción·6, Cobranza·2; **14/14 verde**). **Pendientes:** (a) **color-contrast** (a11y serious, sistémico) → decisión de diseño de Beto sobre los tokens `/40`–`/50`; (b) los 5 smoke de RDB de S1 (pulido pantalla-por-pantalla); (c) más empresas + S4 (¿CI? ¿entorno desechable para tests de mutación?).
**Dueño:** Beto
**Creada:** 2026-06-26
...
- **2026-06-26** — S2 (a11y). **3 violaciones WCAG críticas resueltas** (0 críticas restantes en axe): `button-name` de los botones Refresh icon-only de Tareas (`tasks-module`) y Empresas (`settings/empresas`) + `label` del `<input type=date>` "Al corte" del inventario RDB → aria-labels (cero cambio visual). Helper de a11y mejorado (imprime selector + HTML por nodo). **Pendiente:** `color-contrast` (serious, sistémico: ~13/3/416/17 nodos de texto con opacidad `/40`–`/50`) → subir el contraste de esos tokens es **decisión de diseño de Beto** (afecta toda la app). Observación: el patrón "Refresh icon-only sin aria-label" probablemente vive en más páginas no auditadas (deuda latente → S3 o lint rule).
- **2026-06-26** — S3 (extender cobertura). Smoke read-only parametrizado de los módulos operativos DILESA: **13 superficies** (Compras: órdenes/requisiciones/cotizaciones/recepciones/costo-materiales; Construcción: obras/contratos/contratistas/prototipos/estimaciones/costeo; Cobranza: pagos/aging) → `auth-dilesa-modulos.spec.ts`. **14/14 verde a la primera.** Patrón: navega + verifica render sin crash (h1 del módulo o acceso-restringido), tolerante a sin-datos/sin-permiso, sin asumir textos/datos; excluye rutas de captura (`/nuevo`). Confirma que los 3 hubs + sus routed-tabs cargan sano para un usuario con lectura.
