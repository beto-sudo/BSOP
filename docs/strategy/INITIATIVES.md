...
>
> **Última actualización:** 2026-04-26 (`detail-page` → done [PR #217 mergeado]; `data-table` proposed → done [sesión nocturna: 8 PRs A-G mergeados — `lib/format/`, `<DataTable>` foundation + ADR-010, golden paths Cortes/Ventas/Inventario stack/Productos/Compras/DILESA terrenos/RH stack]; pendientes documentados como Fase 2 en planning doc)
...
| Data Table compartido (UI)           | `data-table`          | 2026-04-26 | PRs #218 (paso 0), #219 (lib/format/), #220 (foundation + ADR-010 + Cortes), #221 (Ventas), #222 (Inventario stack + Productos), #223 (Compras), #224 (DILESA terrenos), #225 (RH). 9 áreas migradas (~17 tablas), `<DataTable>` + tanstack core, lib/format/ centralizado, ADR-010 con DT1-DT8. Fase 2: documentos, tasks, juntas, productos/analisis, DILESA proyectos/prototipos/anteproyectos como follow-up incremental — ver bitácora. |
