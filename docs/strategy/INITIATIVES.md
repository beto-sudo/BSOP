...
> Cowork/CC).
>
> **Última actualización:** 2026-04-27 (Iniciativa `shared-modules-refactor` **cerrada** con Sub-PR 5: `empleado-detail-shared` mergeado. RDB hereda contrato/finiquito + 5 features extras (beneficiarios Art. 501, documentos, notas, multi-tel, contacto emergencia). `PATRON_DILESA` (fallback hardcoded) eliminado; ADR-011 amplía a SM6 (cero fallback en módulos legales). Datos fiscales de empresa obligatorios para alta de empleado y generación de contrato/finiquito — RDB queda pendiente de cargar CSF para destrabar uso real. Próximo: arranca `forms-pattern` en cola UI.)
...
| Forms pattern (UI)          | `forms-pattern`            | todas    | n/a (UI)                    | proposed    | Cerrar alcance v1 al arrancar — **siguiente en cola UI (libre tras cierre de `shared-modules-refactor`)**          | 2026-04-27           |
