...
> Cowork/CC).
>
> **Última actualización:** 2026-04-27 (Iniciativa `empleados-multi-puesto` **cerrada** en 4 PRs: #252 DB N:M + ADR-013, #254 trigger sync, #255 rename UI Empleados→Personal + redirects 301, _este PR_ cleanup datos. Modelo escalar `empleados.puesto_id` migrado a N:M `empleados_puestos`; los 3 operadores tienen sus 3 puestos (Accionista + Comité + Consejo) cargados en RDB y DILESA. Próximo: Sub-PR 5 de `shared-modules-refactor` (`empleados-detail-audit`).)
