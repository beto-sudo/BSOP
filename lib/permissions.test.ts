...
describe('route maps', () => {
  ...
  it('maps nav hrefs to empresa slugs (sanren/family/travel/health/peptides all → sanren)', () => {
    ...
    expect(ROUTE_TO_EMPRESA['/peptides']).toBe('sanren');
  });
});
...
const EXPECTED_DB_MODULE_SLUGS = new Set<string>([
  ...
  'dilesa.proveedores',
  'dilesa.portafolio',
  // Sub-slugs del hub Portafolio (ADR-030): Inventario / Evaluación / Prediales.
  ...
  // Permiso específico de autorización de asignación (Dirección + Nelcy).
  // Sub-slug nuevo de la iniciativa hold+cola (Fase 2 capture).
  ...
  'rdb.inventario',
  // RDB Inventario sub-slugs (mismo patrón).
