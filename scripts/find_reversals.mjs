...
if (error) {
  ...
  console.log(`Found ${negPagos.length} negative payments across ${new Set(negPagos.map(p => p.order_id)).size} orders\n`);
  ...
  for (const ped of pedidos || []) {
    ...
    console.log(`  Status: ${ped.status} | Total: $${ped.total_amount} | Discount: $${ped.total_discount} | Net payments: $${net}`);
