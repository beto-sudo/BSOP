...
<!--
  ...
  Last regenerated: 2026-07-02T22:54:22Z
...
**`dilesa.activo_espacio_publicitario`**

- `dilesa_activo_espectacular_updated_at`: `CREATE TRIGGER dilesa_activo_espectacular_updated_at BEFORE UPDATE ON dilesa.activo_espacio_publicitario FOR EACH ROW EXECUTE FUNCTION core.fn_set_updated_at()`
...
- `activo_espacio_publicitario_subtipo_check`: `CHECK ((subtipo = ANY (ARRAY['espectacular'::text, 'unipolar'::text, 'padel'::text, 'valla'::text, 'otro'::text])))`
**`dilesa.activo_infraestructura`**
...
- `activos_tipo_check`: `CHECK ((tipo = ANY (ARRAY['terreno'::text, 'espacio_publicitario'::text, 'casa'::text, 'local'::text, 'plaza'::text, 'edificio'::text, 'nave'::text, 'departamento'::text, 'lote'::text, 'infraestructura'::text, 'cara'::text, 'espectacular'::text, 'unipolar'::text])))`
**`dilesa.construccion`**
...
#### `rdb.fn_pos_admin_upsert_zona(p_empresa_id uuid, p_nombre text, p_orden integer, p_activa boolean, p_id uuid)`

```sql
CREATE OR REPLACE FUNCTION rdb.fn_pos_admin_upsert_zona(p_empresa_id uuid, p_nombre text, p_orden integer DEFAULT 100, p_activa boolean DEFAULT true, p_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'rdb', 'core', 'public'
