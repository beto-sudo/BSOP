...
<!--
  ...
  Last regenerated: 2026-07-02T22:45:38Z
...
- **uso_id** `text` NOT NULL — PK
- **modelo** `text` NOT NULL
- **nota** `text`
- **actualizado_por** `uuid` — FK → `core.usuarios(id)`
...
- **costo_estimado_usd** `numeric` NOT NULL DEFAULT `0`
...
- **compras_emision_requiere_direccion** `boolean` NOT NULL DEFAULT `false`
...
- **usuario_id** `uuid` NOT NULL — PK, FK → `core.usuarios(id)`
- **empresa_id** `uuid` NOT NULL — PK, FK → `core.empresas(id)`
- **modulo_id** `uuid` NOT NULL — PK, FK → `core.modulos(id)`
...
- **plantilla_id** `uuid` NOT NULL — PK, FK → `core.rol_plantillas(id)`
- **modulo_id** `uuid` NOT NULL — PK, FK → `core.modulos(id)`
...
### `core.v_usuarios_directorio` _(view)_
...
- **activo_id** `uuid` NOT NULL — FK → `dilesa.activos(id)`
- **documento_id** `uuid` NOT NULL — FK → `erp.documentos(id)`
- **rol** `text` NOT NULL DEFAULT `'escritura'::text`
...
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **caras_detalle** `jsonb` NOT NULL DEFAULT `'[]'::jsonb`
- **dueno_terreno** `text`
- **subtipo** `text` NOT NULL DEFAULT `'espectacular'::text`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **movimiento_id** `uuid` NOT NULL — FK → `dilesa.activo_movimientos(id)`
- **activo_id** `uuid` NOT NULL — FK → `dilesa.activos(id)`
- **rol** `text` NOT NULL
...
- **tipo** `text` NOT NULL
- **fecha** `date` NOT NULL
- **documento_id** `uuid` — FK → `erp.documentos(id)`
...
- **deleted_at** `timestamp with time zone`
- **modalidad** `text`
- **destino_id** `uuid` — FK → `dilesa.portafolio_destinos(id)`
- **zona** `text`
- **etiqueta** `text`
...
- **fecha_terminada** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
...
- **personal_a_disposicion** `boolean` NOT NULL DEFAULT `false`
- **retencion_fiscal_isr_pct** `numeric` NOT NULL DEFAULT `0`
- **retencion_fiscal_iva_pct** `numeric` NOT NULL DEFAULT `0`
- **repse_override_at** `timestamp with time zone`
- **repse_override_por** `uuid`
- **repse_override_motivo** `text`
...
- **meta_dias** `numeric` NOT NULL
- **activa** `boolean` NOT NULL DEFAULT `true`
- **nota** `text`
- **editado_por** `uuid` — FK → `core.usuarios(id)`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
...
- **activa** `boolean` NOT NULL DEFAULT `true`
- **isai_pct** `numeric` NOT NULL DEFAULT `0.03`
- **muni_certificacion_planos** `numeric` NOT NULL DEFAULT `165`
- **muni_copias_fotostaticas** `numeric` NOT NULL DEFAULT `56`
- **muni_avaluo_previo** `numeric` NOT NULL DEFAULT `566`
- **muni_valuacion_catastral** `numeric` NOT NULL DEFAULT `1200`
- **muni_derechos** `numeric` NOT NULL DEFAULT `850`
- **rp_clg** `numeric` NOT NULL DEFAULT `575`
- **rp_aviso_preventivo** `numeric` NOT NULL DEFAULT `0`
- **apertura_umbral_cuota_fija** `numeric` NOT NULL DEFAULT `820000`
- **apertura_cuota_fija** `numeric` NOT NULL DEFAULT `765`
- **otros_cnpr_por_derechohabiente** `numeric` NOT NULL DEFAULT `1000`
- **otros_aviso_definitivo** `numeric` NOT NULL DEFAULT `103`
- **otros_forma_isai** `numeric` NOT NULL DEFAULT `400`
- **otros_copia_certificada** `numeric` NOT NULL DEFAULT `1500`
- **otros_plano** `numeric` NOT NULL DEFAULT `1200`
- **otros_kinegrama** `numeric` NOT NULL DEFAULT `200`
- **notas** `text`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **deleted_at** `timestamp with time zone`
- **categoria** `text` NOT NULL
- **muni_forma_isai** `numeric` NOT NULL DEFAULT `0`
- **muni_no_adeudo_simas** `numeric` NOT NULL DEFAULT `0`
- **muni_valuacion_catastral_pct** `numeric` NOT NULL DEFAULT `0`
- **otros_avaluo** `numeric` NOT NULL DEFAULT `0`
- **otros_cnpc** `numeric` NOT NULL DEFAULT `0`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **config_id** `uuid` NOT NULL — FK → `dilesa.gastos_notariales_config(id)`
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **tipo** `text` NOT NULL
- **orden** `integer` NOT NULL
- **limite_inferior** `numeric` NOT NULL
- **limite_superior** `numeric`
- **valor_beneficio** `numeric` NOT NULL
- **valor_particular** `numeric` NOT NULL
...
> Estimaciones de monto por contrato de obra no-vivienda (anticipo + numeradas + finiquito; etiqueta libre). IVA (8% frontera / 16% excepción) desglosado donde esté especificado. Vivienda usa dilesa.estimaciones (por tareas terminadas, ADR-033) — esta tabla es para monto directo por avance.
...
- **pagada_at** `timestamp with time zone`
- **tope_override_motivo** `text`
- **amortizacion_aplicada** `numeric` NOT NULL DEFAULT `0`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **cuenta_id** `uuid` NOT NULL — FK → `dilesa.cuentas_prediales(id)`
...
- **convenio_id** `uuid` — FK → `dilesa.prediales_convenios(id)`
- **estado** `text` NOT NULL DEFAULT `'pendiente'::text`
- **fecha_pago** `date`
- **monto_pagado** `numeric`
- **pagado_por** `uuid` — FK → `core.usuarios(id)`
- **notas** `text`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
...
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **construccion_id** `uuid` NOT NULL — FK → `dilesa.construccion(id)`
- **estado** `text` NOT NULL DEFAULT `'recibida'::text`
- **fecha_recepcion** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
- **recibido_por_user_id** `uuid` — FK → `core.usuarios(id)`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **recepcion_id** `uuid` NOT NULL — FK → `dilesa.recepcion_obra(id)`
- **fecha_visita** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
- **resultado** `text` NOT NULL
- **observaciones** `text`
- **fecha_reprograma** `date`
- **compromiso_contratista** `text`
- **registrado_por_user_id** `uuid` — FK → `core.usuarios(id)`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **deleted_at** `timestamp with time zone`
...
- **fecha_seguro_calidad** `date`
- **fecha_paquete_ruv** `date`
- **problema_zcu** `boolean` NOT NULL DEFAULT `false`

### `dilesa.v_ac_encuestas_pendientes` _(view)_
...
### `dilesa.v_ac_encuestas_respondidas` _(view)_
...
- **empresa_id** `uuid`
- **nps** `integer`
- **calif_vivienda** `integer`
- **calif_proceso** `integer`
- **comentario** `text`
- **canal** `text`
- **respondida_at** `timestamp with time zone`
- **respondida_fecha** `date`
- **cliente** `text`
- **unidad** `text`
- **proyecto_id** `uuid`
- **proyecto** `text`
...
- **empresa_id** `uuid`
- **encuestas_respondidas** `bigint`
- **encuestas_total** `bigint`
- **nps_prom** `numeric`
- **calif_vivienda_prom** `numeric`
- **calif_proceso_prom** `numeric`

### `dilesa.v_ac_obras_por_recibir` _(view)_
...
### `dilesa.v_ac_ventas_entrega` _(view)_
...
### `dilesa.v_fase_benchmark` _(view)_
...
### `dilesa.v_ruv_proyectos_disponibles` _(view)_
...
### `dilesa.v_venta_fase_duraciones` _(view)_
...
- **fecha_salida** `date`
- **dias_en_fase** `integer`
- **es_tramo_abierto** `boolean`
- **es_negativo** `boolean`

### `dilesa.v_ventas_lista_antiguedad` _(view)_

- **venta_id** `uuid`
- **empresa_id** `uuid`
- **fase_actual** `text`
- **fase_posicion** `integer`
- **fecha_fase_actual** `date`
- **dias_en_fase** `integer`

### `dilesa.v_ventas_pipeline_antiguedad` _(view)_
...
- **adjunto_id** `uuid` — FK → `erp.adjuntos(id)`
...
- **modelo** `text`
- **error_detalle** `text`
- **ejecutado_por** `uuid` — FK → `core.usuarios(id)`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **adjunto_acuse_id** `uuid` — FK → `erp.adjuntos(id)`
...
- **notario_id** `uuid` — FK → `erp.proveedores(id)`
...
- **cd_spread_ordinario_pct** `numeric` DEFAULT `4`
...
- **valor_real_venta_dilesa** `numeric`
- **valor_facturado** `numeric`
- **monto_nota_credito** `numeric`
- **descuento_precio** `numeric`
- **descuento_equipamiento** `numeric`
- **descuento_gastos_escrituracion** `numeric`
- **descuento_nota_credito** `numeric`
- **apoyo_infonavit** `numeric`
- **descuento_maximo_autorizado** `numeric`
- **cd_interes_moratorio_pct** `numeric`
- **promocion_id** `uuid` — FK → `dilesa.promociones(id)`
- **notif_escrituracion_at** `timestamp with time zone`
- **poliza_garantia_expedida_at** `timestamp with time zone`
- **desglose_precio** `jsonb`
- **precio_base** `numeric`
- **incremento_credito** `numeric`
- **promocion_gastos_monto** `numeric`
- **valor_excedente_terreno** `numeric`
- **valor_frente_verde** `numeric`
- **valor_esquina** `numeric`
- **valor_venta_futuro** `numeric`
...
- **tiene_propiedad** `boolean` NOT NULL DEFAULT `false`
- **gastos_notariales_desglose** `jsonb`
- **fecha_pre_entrega** `date`
- **fecha_entrega** `date`
- **valor_catastral** `numeric`
- **saldo_gastos_resolucion** `text`
- **saldo_gastos_monto** `numeric`
- **saldo_gastos_autorizado_por** `uuid`
- **saldo_gastos_at** `timestamp with time zone`
- **descuento_valor_base** `numeric`
- **descuento_valor_base_motivo_id** `uuid` — FK → `dilesa.descuento_motivos(id)`
- **descuento_valor_base_detalle** `text`
- **descuento_valor_base_autorizado_por** `uuid` — FK → `core.usuarios(id)`
- **descuento_valor_base_autorizado_at** `timestamp with time zone`
- **venta_origen_id** `uuid` — FK → `dilesa.ventas(id)`
...
- **tipo** `text`
- **descripcion** `text`
- **fecha** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
- **costo** `numeric`
...
- **sustituido_at** `timestamp with time zone`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **arrendamiento_id** `uuid` NOT NULL — FK → `erp.arrendamientos(id)`
- **linea_id** `uuid` — FK → `erp.arrendamiento_lineas(id)`
- **cxc_cargo_id** `uuid` — FK → `erp.cxc_cargos(id)`
- **periodo** `text`
- **tipo** `text` NOT NULL
...
- **arrendamiento_id** `uuid` NOT NULL — FK → `erp.arrendamientos(id)`
- **monto** `numeric` NOT NULL
- **deposito_naturaleza** `text` NOT NULL DEFAULT `'garantia_reembolsable'::text`
...
- **movimiento_bancario_id** `uuid` — FK → `erp.movimientos_bancarios(id)`
...
- **arrendamiento_id** `uuid` NOT NULL — FK → `erp.arrendamientos(id)`
- **activo_id** `uuid` NOT NULL — FK → `dilesa.activos(id)`
- **tipo_operacion_fiscal** `text` NOT NULL DEFAULT `'arrendamiento_inmueble'::text`
- **renta_subtotal** `numeric` NOT NULL
- **regimen_iva** `text` NOT NULL DEFAULT `'tasa_8'::text`
- **iva_tasa_pct** `numeric` NOT NULL DEFAULT `8`
- **iva_fundamento** `text`
- **lugar_expedicion** `text`
- **iva_validado_por** `uuid`
- **iva_validado_at** `timestamp with time zone`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **linea_id** `uuid` NOT NULL — FK → `erp.arrendamiento_lineas(id)`
- **vigencia_inicio** `date` NOT NULL
- **vigencia_fin** `date`
- **monto** `numeric` NOT NULL
- **inpc_aplicado** `numeric`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **arrendatario_persona_id** `uuid` NOT NULL — FK → `erp.personas(id)`
- **pagador_persona_id** `uuid` — FK → `erp.personas(id)`
- **receptor_fiscal_persona_id** `uuid` — FK → `erp.personas(id)`
- **arrendador_persona_id** `uuid` — FK → `erp.personas(id)`
- **folio** `text`
- **tipo_plazo** `text` NOT NULL DEFAULT `'plazo'::text`
- **fecha_inicio** `date`
- **fecha_fin** `date`
- **dia_corte** `integer`
- **esquema_incremento** `text` NOT NULL DEFAULT `'inpc_mas_pct'::text`
- **pct_adicional** `numeric` NOT NULL DEFAULT `2.0`
- **inpc_base_anio** `integer`
- **inpc_base_mes** `integer`
- **tipo_renovacion** `text` NOT NULL DEFAULT `'manual'::text`
- **penalizacion_terminacion_meses** `numeric` NOT NULL DEFAULT `2`
- **requiere_fiador** `boolean` NOT NULL DEFAULT `false`
- **fiador_persona_id** `uuid` — FK → `erp.personas(id)`
...
- **diferencia** `numeric`
- **observaciones** `text`
- **fecha_operativa** `date` DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
...
- **cuenta_padre_id** `uuid` — FK → `erp.cuentas_contables(id)`
- **codigo_agrupador_sat** `text`
- **afectable** `boolean` NOT NULL DEFAULT `true`
- **activa** `boolean` NOT NULL DEFAULT `true`
- **origen** `text` NOT NULL DEFAULT `'contpaqi'::text`
- **notas** `text`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **deleted_at** `timestamp with time zone`
...
- **notif_alta_at** `timestamp with time zone`
- **notif_baja_at** `timestamp with time zone`
...
- **fuente** `text` NOT NULL DEFAULT `'INEGI'::text`
- **estado** `text` NOT NULL DEFAULT `'capturado'::text`
- **capturado_por** `uuid`
- **aprobado_por** `uuid`
...
- **estado** `text` NOT NULL DEFAULT `'borrador'::text`
- **fecha_programada** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
...
- **metodo** `text`
- **referencia** `text`
- **fecha_pago** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
...
### `erp.presupuesto_baseline_partidas`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **baseline_id** `uuid` NOT NULL — FK → `erp.presupuesto_baselines(id)`
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **partida_id** `uuid` NOT NULL — FK → `erp.presupuesto_partidas(id)`
- **monto_baseline** `numeric` NOT NULL DEFAULT `0`
- **concepto_texto** `text`
- **etapa** `text`

### `erp.presupuesto_baselines`
...
- **autorizado_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
...
- **costo** `numeric`
- **precio_venta** `numeric`
- **fecha_inicio** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
- **fecha_fin** `date`
- **vigente** `boolean` NOT NULL DEFAULT `true`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
...
- **total** `numeric`
- **estado_id** `uuid`
- **fecha_entrada** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
- **fecha_entrega** `date`
- **vin_externo** `text`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone`
...
### `erp.v_presupuesto_reconciliacion` _(view)_
...
- **concepto_texto** `text`
- **etapa** `text`
- **monto_baseline** `numeric`
- **cambios_netos** `numeric`
- **vigente** `numeric`
- **drift** `numeric`
...
- **comision** `numeric`
- **estado_id** `uuid`
- **fecha_venta** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **updated_at** `timestamp with time zone`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **estacion_id** `uuid` NOT NULL — FK → `rdb.pos_estaciones(id)`
- **ubicacion** `text`
- **tipo_venta** `text` NOT NULL DEFAULT `'normal'::text`
- **estado** `text` NOT NULL DEFAULT `'abierta'::text`
- **cliente_nombre** `text`
- **playtomic_folio** `text`
- **cuenta_origen_id** `uuid` — FK → `rdb.pos_cuentas(id)`
- **abierta_por** `uuid` NOT NULL — FK → `erp.empleados(id)`
...
- **updated_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
- **notas** `text`
- **fecha_operativa** `date` NOT NULL DEFAULT `((now() AT TIME ZONE 'America/Matamoros'::text))::date`
- **folio** `integer`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **cuenta_id** `uuid` — FK → `rdb.pos_cuentas(id)`
- **item_id** `uuid` — FK → `rdb.pos_items(id)`
- **pago_id** `uuid` — FK → `rdb.pos_pagos(id)`
- **evento** `text` NOT NULL
- **actor_empleado_id** `uuid` — FK → `erp.empleados(id)`
- **actor_empleado_nombre** `text`
- **actor_auth_uid** `uuid`
- **estacion_id** `uuid` — FK → `rdb.pos_estaciones(id)`
- **autorizado_por** `uuid` — FK → `erp.empleados(id)`
- **datos_antes** `jsonb`
- **datos_despues** `jsonb`
- **razon** `text`
...
- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **empresa_id** `uuid` NOT NULL — FK → `core.empresas(id)`
- **cuenta_id** `uuid` NOT NULL — FK → `rdb.pos_cuentas(id)`
- **ronda_id** `uuid` NOT NULL — FK → `rdb.pos_rondas(id)`
- **producto_id** `uuid` NOT NULL — FK → `erp.productos(id)`
...
- **descuento_autorizado_por** `uuid` — FK → `erp.empleados(id)`
- **estado** `text` NOT NULL DEFAULT `'capturado'::text`
- **void_razon** `text`
- **void_por** `uuid` — FK → `erp.empleados(id)`
- **notas** `text`
...
- **cambio** `numeric`
- **referencia** `text`
- **reversa_de** `uuid` — FK → `rdb.pos_pagos(id)`
- **registrado_por** `uuid` NOT NULL — FK → `erp.empleados(id)`
- **client_action_id** `uuid` NOT NULL
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
...
- **capturada_por** `uuid` NOT NULL — FK → `erp.empleados(id)`
- **estacion_id** `uuid` NOT NULL — FK → `rdb.pos_estaciones(id)`
- **client_action_id** `uuid` NOT NULL
- **created_at** `timestamp with time zone` NOT NULL DEFAULT `now()`
...
> Per-product sales aggregates per RDB corte (Waitry POS). security_invoker=true; enforces RLS of the caller against waitry_pedidos/waitry_productos. See app/rdb/cortes/page.tsx for consumer.
...
### `rdb.v_ventas_canonicas` _(view)_
...
> Propiedades de la familia (casa principal y futuras). Iniciativa sanren-servicios.
...
> Recibos de servicios (≥1 por servicio/mes posible). Derivaciones en sanren.v_recibos. Iniciativa sanren-servicios.

- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **servicio_id** `uuid` NOT NULL — FK → `sanren.servicios(id)`
- **periodo** `date` NOT NULL
...
- **tarifa** `text`
- **extraccion** `jsonb`
- **extraccion_at** `timestamp with time zone`

### `sanren.servicios`

> Servicios contratados por propiedad (catálogo extensible por texto). Iniciativa sanren-servicios.

- **id** `uuid` NOT NULL DEFAULT `gen_random_uuid()` — PK
- **propiedad_id** `uuid` NOT NULL — FK → `sanren.propiedades(id)`
- **tipo** `text` NOT NULL
- **proveedor** `text`
- **numero_cuenta** `text`
- **numero_medidor** `text`
- **unidad_consumo** `text`
- **tiene_produccion** `boolean` NOT NULL DEFAULT `false`
...
### `sanren.v_recibos` _(view)_

> Recibos con derivaciones (consumo/producción del periodo, costo unitario, saldo neto solar, Δ MoM) + campos de extracción IA. Iniciativa sanren-servicios.
...
- **lectura_consumo_anterior** `numeric`
- **lectura_produccion_anterior** `numeric`
- **monto_anterior** `numeric`
- **consumo_periodo** `numeric`
- **produccion_periodo** `numeric`
- **costo_unitario** `numeric`
- **saldo_neto** `numeric`
- **delta_monto_mom** `numeric`
