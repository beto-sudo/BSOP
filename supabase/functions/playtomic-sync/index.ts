...
serve(async (req) => {
  ...
  try {
    ...
    if (resourcesMap.size > 0) {
      await supabase.schema("playtomic").from("resources").upsert(Array.from(resourcesMap.values()), { onConflict: "resource_id" });
    }
    ...
    if (playersMap.size > 0) {
      ...
      await supabase.schema("playtomic").from("players").upsert(playersArr, { onConflict: "playtomic_id" });
    }
    ...
    for (let i = 0; i < bookingsData.length; i += chunkSize) {
      ...
      const { error } = await supabase.schema("playtomic").from("bookings").upsert(chunk, { onConflict: "booking_id" });
    ...
    for (let i = 0; i < participantsData.length; i += chunkSize) {
      ...
      const { error } = await supabase.schema("playtomic").from("booking_participants").upsert(chunk, { onConflict: "booking_id, player_id" });
      if (error) throw new Error(`Participants Upsert Error: ${error.message}`);
    }

    // 5. Finalize Log
    await supabase.schema("playtomic").from("sync_log").update({
    ...
    return new Response(JSON.stringify({ success: true, fetched: allBookings.length }), { headers: { "Content-Type": "application/json" } });
