...
async function upsertCorte(rowId: string, v: Record<string, any>): Promise<any> {
  const payload = {
    ...
    p_caja_nombre:
      typeof v['Caja'] === 'object' ? (v['Caja']?.name ?? String(v['Caja'])) : (v['Caja'] ?? null),
...
async function upsertMovimiento(rowId: string, v: Record<string, any>): Promise<any> {
  const payload = {
    ...
    p_nota: v['Nota'] ?? null,
    p_registrado_por:
      typeof v['Registró'] === 'object'
        ? (v['Registró']?.name ?? String(v['Registró']))
        : (v['Registró'] ?? null),
  };
  ...
  try {
    ...
    for (const { rowId, values } of cortes) {
      ...
      if (res.ok) c_upd++;
        ...
        c_err++;
        log.push(`  ❌ Corte ${values['ID Corte']}: ${res.error}`);
      }
    ...
    for (const { rowId, values } of movs) {
      ...
      if (res.ok) m_upd++;
        ...
        m_err++;
        log.push(`  ❌ Movimiento ${rowId}: ${res.error}`);
      }
      await new Promise((r) => setTimeout(r, 50));
    }
