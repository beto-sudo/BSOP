...
-- Índice único en coda_id para upsert por folio de Coda
CREATE UNIQUE INDEX IF NOT EXISTS rdb_cortes_coda_id_idx
  ON rdb.cortes (coda_id)
  WHERE coda_id IS NOT NULL;

-- ============================================================
-- 2. Función rdb.upsert_corte
--    Equivalente a la que existía en waitry/caja, ahora en rdb.
--    Upsert por coda_id si viene, o por (caja_nombre, hora_inicio).
-- ============================================================
...
BEGIN
  ...
  -- Upsert: buscar por coda_id si viene, si no por (caja_id, hora_inicio)
  ...
  ELSE
    -- Sin coda_id: upsert por (caja_id, hora_inicio)
    INSERT INTO rdb.cortes (
      coda_id, corte_nombre, caja_id, caja_nombre,
      estado, turno, responsable_apertura, responsable_cierre,
      observaciones, efectivo_inicial, efectivo_contado,
...
-- ============================================================
-- 3. Actualizar v_cortes_lista para incluir corte_nombre real
-- ============================================================
