...
-- 2. Extender inventario_movimientos con columnas faltantes
--    (tabla ya existe con: id, producto_id, tipo, cantidad, costo_unitario, referencia_tipo, referencia_id, notas, creado_por, created_at)
...
-- Sincronizar fecha con created_at en filas existentes
UPDATE rdb.inventario_movimientos SET fecha = created_at WHERE fecha IS NULL;

-- Extender el CHECK de tipo para incluir los valores del módulo
-- (primero drop la constraint existente si existe, luego agregar la nueva)
ALTER TABLE rdb.inventario_movimientos
...
-- ============================================================
-- 3. Vista de stock calculado por producto padre
-- ============================================================
...
movimientos_agg AS (
  SELECT
    ...
    SUM(CASE WHEN tipo = 'ajuste'                                            THEN 0 ELSE 0 END)         AS _ajuste_legacy  -- ajuste legacy: neutro hasta reclasificar
...
-- ============================================================
-- 4. Vista de grupos de productos (padre + hijos)
-- ============================================================
