-- EDITED 2026-04-23 (drift-1.5): rdb.inventario_movimientos / waitry_* /
-- productos are ambient. SQL functions validate references at create time, so
-- the whole CREATE FUNCTION must be skipped when the sources are absent.
-- fn_inventario_al_corte: stock snapshot at a given timestamp
-- Uses only movimientos and pedidos created_at <= p_fecha

DO $do$
BEGIN
  IF to_regclass('rdb.inventario_movimientos') IS NULL
     OR to_regclass('rdb.waitry_productos') IS NULL
     OR to_regclass('rdb.waitry_pedidos') IS NULL
     OR to_regclass('rdb.productos') IS NULL THEN
    RETURN;
  END IF;
