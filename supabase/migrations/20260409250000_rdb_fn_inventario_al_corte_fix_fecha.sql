-- Fix: usar columna `fecha` (fecha real del movimiento) en lugar de `created_at` (fecha de inserción del backfill)
-- Problema: los 816 movimientos históricos se insertaron el 9-abr-2026 en batch desde CSV,
-- pero su columna `fecha` tiene la fecha real (desde ago-2025). El filtro usaba `created_at`
-- que siempre era 2026-04-09, haciendo que cualquier corte anterior al 9-abr mostrara 0 entradas.
...
movimientos_agg AS (
  ...
  -- KEY FIX: usar COALESCE(fecha, created_at) para respetar la fecha real del movimiento
